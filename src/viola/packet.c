...
char* packetTypeStr[] = {
    "INT", "FLT", "STR", "CHR", "OBJ", "PKT", "ATR", "VLT", "ARY",
    /*	"STRI",*/
};
...
Packet* borrowPacket() {
    ...
    if (++packetStackIdx > packetStackSize) {
        ...
        fprintf(stderr, "internal error: Packet stack overflow.\n");
        fprintf(stderr, "use ``-pstack'' option to allocate larger stack.\n");
        fprintf(stderr, "Whoops, -pstack doesn't yet exist. Tell Pei that this is needed.\n");
        /* XXX unimplemented */
        exit(1);
    }
...
void returnPacket() {
    ...
    if (packetStackIdx < 0) {
        fprintf(stderr, "internal error: returnPacket() stack underflow...\n");
        return;
    }
    packetp = &packetStack[packetStackIdx--];
...
void returnPackets(n) int n;
{
    while (n-- > 0) {
        if (packetStackIdx < 0) {
            fprintf(stderr, "internal error: returnPackets() stack underflow...\n");
            return;
        }
...
Packet* packetFrom;
{
    /*	char *savep;
            int leaks;
    */
    if ((packetTo->type == PKT_STR) && (packetTo->canFree & PK_CANFREE_STR)) {
