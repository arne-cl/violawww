...
int ASTTypeStrSize;
char* ASTTypeStr[] = {"EXPR_PLUS",  /* the EXPR's must be grouped in the front. */
...
int type;
{
    ...
    if (++ASTStackIdx >= ASTStackSize) {
        ...
        fprintf(stderr, "internal error: AST stack overflow: ASTStackIdx=%d, ASTStackSize=%d.\n",
                ASTStackIdx, ASTStackSize);
        fprintf(stderr, "use ``-astack'' option to allocate larger stack.\n");
        /* XXX unimplemented */
        exit(1);
    }
...
int tab;
{
    ...
    for (; ast; ast = ast->next) {
        ...
        case AST_STRING:
            fprintf(stderr, "info=\"%s\"", (char*)(ast->attribute.info.s));
            break;

        case AST_CHAR:
            fprintf(stderr, "info='%c' (%d)", (char)(ast->attribute.info.c),
                    (int)(ast->attribute.info.c));
            break;

        case AST_FLOAT:
            fprintf(stderr, "info=%f", (float)(ast->attribute.info.f));
            break;

        case AST_INTEGER:
        default:
            fprintf(stderr, "info=%d", (int)(ast->attribute.info.i));
            break;
        }
        putc('\n', stderr);

        if (ast->children)
            printAST(ast->children, tab + 4);
    }
}
