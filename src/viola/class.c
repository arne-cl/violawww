...
void overRideSlotLookUpTable(ClassInfo* cip, int section, SlotInfo** slookup, int slookup_size)
{
    ...
    if (cip->superClass)
        overRideSlotLookUpTable(cip->superClass, section, slookup, slookup_size);
...
int init_class() {
    ...
    */
    for (i = 0; (cip = classList[i]); i++) {
        ...
        for (j = 0, sip = (SlotInfo*)cip->slots[0]; sip[j++].id; cip->common_newcount++)
            ;
        cip->newcount = 0;
        for (j = 0, sip = (SlotInfo*)cip->slots[1]; sip[j++].id; cip->newcount++)
            ;
    }
    ...
    */
    ...
    for (i = 0; (cip = classList[i]); i++) {
        ...
        cip->mhp.bits = (long*)malloc(sizeof(long) * cip->mhp.bitsSize);
    ...
    for (cip = classList; cip; cip++) {
        if ((entry = symID2Str->get(symID2Str, (long)cip->id))) {
            ...
            if (loadFile(buff, &classScript) != -1) {
                ...
                */
                cip->common[SLOT_IDX_classScript] = (long)classScript;
            }
        }
    }
    return 1;
...
char* returnSlotCast(int type)
{
    switch (type) {
    case PTRV:
        return ""; /* don't specify type. let it be implicit */
    case ATTR:
        return "(Attr*)";
    case OBJP:
        return "(VObj*)";
...
int generateSlotAccessors() {
    ...
    for (i = 0; cip = classList[i]; i++) {
        ...
        for (j = 0; j < cip->common_totalcount; j++) {
            ...
            fprintf(fp, "#define GET_%s(o) (%s((ClassInfo*)((o)[0])->common[%d]))\n",
                    (char*)symID2Str->get(symID2Str, sip->id)->val, slotCast, sip->offset);
            fprintf(fp, "#define SET_%s(o,v) (%s((ClassInfo*)((o)[0])->common[%d]=(long)(v)))\n\n",
                    (char*)symID2Str->get(symID2Str, sip->id)->val, slotCast, sip->offset);
        }
        ...
        for (j = 0; j < cip->totalcount; j++) {
            ...
            /*
                                    fprintf(fp,
                                            ...
                                            sip->offset);
            */
            fprintf(fp, "#define GET_%s(o) (%s((o)[%d]))\n",
                    (char*)symID2Str->get(symID2Str, sip->id)->val, slotCast, sip->offset);
            fprintf(fp, "#define SET_%s(o,v) (%s((o)[%d]=(long)(v)))\n\n",
                    (char*)symID2Str->get(symID2Str, sip->id)->val, slotCast, sip->offset);
        }
    }
    fclose(fp);
...
VObj* instantiateObj(long (*slotv)[100][2], int* slotc)
{
    ...
    if (!slotp) {
        fprintf(stderr, "Error: instantiateObj(): no name specified.\n");
        return NULL;
    }
    ...
    if (!slotp) {
        fprintf(stderr, "instantiateObj(): no class specified for obj %s. Using generic.\n",
                objName);
...
int dumpObj(VObj* obj, long (*dumpFunc)(FILE*, const char*, ...), FILE* dumpDest, int filter, int flag)
{
    ...
    for (i = 0; i < cip->totalcount; i++) {
        sip = cip->slookup[i];
        if ((sip->flags & filter) != filter)
            continue;
        if (flag)
            if (sip->val == obj[i])
                continue;

        dumpFunc(dumpDest, "\\%s {", (char*)symID2Str->get(symID2Str, sip->id)->val);
        ...
        case OBJL:
            for (olist = (VObjList*)obj[i]; olist; olist = olist->next) {
                if (olist->o)
                    dumpFunc(dumpDest, "\"%s\"", GET_name(olist->o));
                else
                    dumpFunc(dumpDest, "NULL");
                if (olist->next)
                    dumpFunc(dumpDest, ",");
        ...
        case ARRY: {
            ...
            if (array) {
                for (n = 0; n < array->size; n++) {
                    dumpFunc(dumpDest, "%d ", array->info[n]);
                }
                dumpFunc(dumpDest, "}\n");
            } else {
                dumpFunc(dumpDest, "NULL}\n");
...
void methodMembershipProfile() {
    ...
    for (cip = classList; cip; cip++) {
        ...
        printf("%20s refc=%5ld\t hits=%5ld\t misses=%5ld collision=%5ld\n", (char*)entry->val,
               mhp->refc, mhp->hits, mhp->misses, mhp->collisions);
    }
}
