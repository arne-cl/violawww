...
int txtDisp_HTML_txt_expose(VObj* self, Packet* result, Packet* argv)
{
    ...
    Packet* arg3 = (Packet*)((Attr*)(argv[0].info.a)->next->next->next->val);
    Packet* arg4 = (Packet*)((Attr*)(argv[0].info.a)->next->next->next->next->val);
    ...
    if (tf && (tfed_get_charMask(tf) & MASK_BUTTON)) {
        ...
        if (ref && *ref) {
            ...
            if (keyStat_shift) {
                ...
                if (mesgObj) {
                    mesg = (char*)malloc(sizeof(char) * (strlen(ref) + 10));
                    strcpy(mesg, "Dest: ");
                    strcat(mesg, ref);
                    sendMessage1N1str(mesgObj, "show", mesg);
                    free(mesg);
                } else {
                ...
                drawLineOffset(tf, tf->current_row - tf->screen_row_offset, 0);
                XFlush(display);

                sendMessage1_result(GET__parent(self), "findTop", &evalResult);
...
int txtDisp_HTML_header_A(VObj* self, Packet* result, Packet* argv)
{
    ...
    if (pk) {
        if (pk->info.s) {
            ...
            if (inset)
                strcat(buff, inset);
            SET__varList(self, setVariable_id_STR(varlist, STR_text, saveString(buff), 1));
            free(inset);
        } else {
            SET__varList(self, setVariable_id_STR(varlist, STR_text, inset, 1));
        }
    } else {
        SET__varList(self, setVariable_id_STR(varlist, STR_text, inset, 1));
    }
...
int mode;
{
    ...
    for (j = 0; j < matRows; j++) {
        ...
        for (i = 0; i < matCols; i++) {
            ...
            if (mat->type & TABLE_CELL_TYPE_THTD || mat->type & TABLE_CELL_TYPE_TB) {
                if (mode == 1) {
                    printf("[%d](%d: \"%s\" %d,%d,%d,%d) ", i, mat->type, GET_name(mat->obj),
                           mat->x, mat->y, mat->width, mat->height);
                } else {
                    printf("[%d](%d: %d,%d,%d,%d) ", i, mat->type, mat->x, mat->y, mat->width,
                           mat->height);
                }
            }
        }
...
int HTMLTableFormater(VObj* self, VObj* titleObj, int titleTopP)
{
    ...
    for (; olist; olist = olist->next) {
        ...
        if (type & TABLE_CELL_TYPE_TR) {
            ...
            if (GET_window(olist->o))
                GLUpdateSize(!GET__parent(olist->o), GET_window(olist->o), GET_width(olist->o) - 1,
                             GET_height(olist->o));
        }
        lastCellType = type;
    }
    ...
    matrix = (TableMatrixCell*)malloc(sizeof(struct TableMatrixCell) * matRows * maxCols);
    ...
    for (row = 0; row < matRows; row++) {
        ...
        for (col = 0; col < maxCols; col++) {
            ...
            if (mat->type & TABLE_CELL_TYPE_THTD) {
                ...
                sendMessage1chr_result(mat->obj, 'r', &evalResult);
        ...
        for (col = 0; col < maxCols; col++) {
            ...
            if (mat->type & TABLE_CELL_TYPE_THTD) {
                ...
                if (GET_window(mat->obj)) {
                    GLUpdateGeometry(!GET__parent(mat->obj), GET_window(mat->obj), GET_x(mat->obj),
