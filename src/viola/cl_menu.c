...
SlotInfo cl_menu_NCSlots[] = {0};
SlotInfo cl_menu_NPSlots[] = {{STR_menuConfig, /* ASCII description of the menu */
                               PTRS | SLOT_RW, (long)".{}"},
                              {STR__menu, /* internal */
                               PTRV, 0},
                              {STR__menuEntries, /* internal */
                               PTRV, 0},
                              {0}};
SlotInfo cl_menu_CSlots[] = {{STR_class, PTRS | SLOT_RW, (long)"menu"},
                             {
                                 STR_classScript,
                                 PTRS,
                                 (long)"\n\
...
SlotInfo cl_menu_PSlots[] = {
    {STR__classInfo, CLSI, (long)&class_menu}, {STR_border, LONG | SLOT_RW, BORDER_MENU}, {0}};

SlotInfo* slots_menu[] = {(SlotInfo*)cl_menu_NCSlots, (SlotInfo*)cl_menu_NPSlots,
                          (SlotInfo*)cl_menu_CSlots, (SlotInfo*)cl_menu_PSlots};

MethodInfo meths_menu[] = {
    /* local methods */
    {STR_config, meth_menu_config},
    {STR_freeSelf, meth_menu_freeSelf},
    {
        STR_geta,
        meth_menu_get,
    },
    {STR_initialize, meth_menu_initialize},
    {STR_processMouseInput, meth_menu_processMouseInput},
    {STR_render, meth_menu_render},
    {STR_seta, meth_menu_set},
    {0}};

ClassInfo class_menu = {
    helper_menu_get, helper_menu_set, slots_menu, /* class slot information	*/
...
long meth_menu_get(VObj* self, Packet* result, int argc, Packet argv[]) {
    return helper_menu_get(self, result, argc, argv, getIdent(PkInfo2Str(argv)));
}
...
long meth_menu_processMouseInput(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (menu_resultn < 0) {
        ...
        data = get_menu_message(menu_entries, menu_result, menu_resultn);
        if (data) {
...
long meth_menu_render(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if ((w = GET_window(self))) {
        ...
        GLDisplayXBM(w, 3, 3, menuPixmap_width, menuPixmap_height, menuPixmap);
        return 1;
    }
    return 0;
}
...
long meth_menu_set(VObj* self, Packet* result, int argc, Packet argv[]) {
    return helper_menu_set(self, result, argc, argv, getIdent(PkInfo2Str(argv)));
}
...
int countDepth(char* str) {
    ...
    for (;;) {
        c = str[i];
        if (c == '\0')
            return 0; /* not menu-entry line.. */
        if (c == '.' && allSpacesBefore) {
            /* it looks like menu entry */
            for (depth = 0; buff[i] == '.'; ++i)
                ++depth;
            return depth;
        }
        if (c != ' ' && c != '\t')
...
int parse_menu_entryScan(char* idata, int* textIndex) {
     ...
     */
    for (;;) {
        if (grabNextMenuLine(textIndex, idata, buff) == 0)
            break;
        depth = countDepth(buff);
        if (depth == 0)
            break; /* malformed entry line */
        if (originalDepth == -1)
            originalDepth = depth;
...
int parse_menu_entryLine(VObj* self, char* entry, char* label, char* returnStr, int* type,
                         int* hasReturnStr, char* accelerator, char* modifier) {
    int i = 0, j, depth;
...
xpa_entry* parse_menu_allocateEntryTree(VObj* self, char* idata, int* textIndex) {
    ...
    if (!(menu = (xpa_entry*)malloc(sizeof(struct xpa_entry_defn) * (menuItemCount + 1))))
        perror("malloc");
    ...
    for (l = 0;;) {
        ...
        accelerator = '\0';
        modifier = MODIFIER_NONE;
        depth = parse_menu_entryLine(self, buff, label, returnStr, &type, &hasReturnStr,
        ...
        if (depth < originalDepth) {
            *textIndex = previousIndex;
            ...
            --l;
            *textIndex = previousIndex;
            subMenu = parse_menu_allocateEntryTree(self, idata, textIndex);
 ...
 * davidh code?
 */
static unsigned long res_color(VObj* self, Display* disp, char* name,
...
int funny_evt(XEvent* evt) {
    ...
    case Expose: {
        VObj* obj;
        HashEntry* hentry = window2Obj->get(window2Obj, (long)eventWindow(*evt));
        if (hentry) {
            obj = (VObj*)hentry->val;
            if (obj) {
                if (verbose)
                    fprintf(stderr, "MENU: expose object = %s.\n", GET_name(obj));
                sendMessage1N4int(obj, "expose", eventExposeX(*evt), eventExposeY(*evt),
                                  eventExposeWidth(*evt), eventExposeHeight(*evt));
            }
        } else {
            if (verbose)
                fprintf(stderr, "MENU: can't find object associated with window.\n");
        }
    }
        return 1;
    case ButtonRelease:
