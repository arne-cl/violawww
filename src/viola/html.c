...
PRIVATE HTStyle default_style = {0, "(Unstyled)", "", HT_NORMAL, 1.0, HT_BLACK, 0,  0, 0, 0,
                                 0, HT_LEFT,      1,  0,         0,   NO,       NO, 0, 0, 0};
...
int html_search(VObj* self, char* keyword)
{
    ...
    if ((htext = html_findHTextByAddress(self, newAddress))) {
        ...
        if (verbose)
            fprintf(stderr, "HTSearch result=%d\n", status);
        if (!status)
            return 0;
        ...
        if (!html_updateHTextTFStruct(oldtf, tf)) {
            fprintf(stderr, "Error: html_updateHTextTFStruct() failed\n");
        }
        if (oldtf) {
            if (oldtf->firstp)
                freeNodeLines(oldtf);
...
void split_line ARGS2(HText*, text, int, split) {
    ...
    printf("::>>");
    dumpTFCArray(DUMP_CHAR | DUMP_FONT, text->tbuff, text->tbuffTagInfo, text->tbuffTagInfoCount);
    printf("<<\n");
#endif
    /* make lineNode
     */
    newp = (TFLineNode*)Vmalloc(text->tfstruct->mg, sizeof(struct TFLineNode));
    if (!newp) {
        fprintf(stderr, "malloc failed\n");
        return;
    }
...
#ifdef VERBOSE_SPLIT_LINE
    printf("made line:>>");
    dumpTFCArray(DUMP_CHAR | DUMP_FONT, newp->linep, newp->tagInfo, newp->tagInfoCount);
    printf("<<\n");
#endif
     ...
     */
    if (text->currentp) {
        if (text->currentp->next)
            text->currentp->next->prev = newp;
...
PUBLIC void HText_setStyle ARGS2(HText*, text, HTStyle*, style) {
    ...
    if (TRACE) {
        fprintf(stderr, "HTML: Changing to style %s (%d). fontID=%d\n", style->name, style->font,
                text->fontID);
    }
    ...
    if (1) {
        ...
        if (TRACE) {
            fprintf(stderr, "HTML: New fontID=%d    l=%f r=%f\n", text->fontID, style->leftIndent,
                    style->rightIndent);
        }
    }
...
**	-------------------------------------
*/
PUBLIC void NOT_HText_appendCharacter ARGS2(HText*, text, char, ch) { printf("- %c\n", ch); }
...
PUBLIC void HText_appendCharacter ARGS2(HText*, text, char, ch) {
    ...
    if (ch == '\t') {
        ...
        if (style->tabs) {
            ...
            if (spaceWidth <= 0) {
                fprintf(stderr, "HTML: S.O.S. font width of 'n' == 0!\n");
        ...
        if (target > limit) {
            ...
            if (spaceWidth <= 0) {
                fprintf(stderr, "HTML: Eh? How is it that I think the font width of ' ' is 0? "
                                "Please inform Pei about this insanity.\n");
            ...
            for (i = target - here - spaceWidth; i > 0; i -= spaceWidth) {
                ...
                bcopy(&spacetfc, tfcp, sizeof(struct TFChar));
                tfcp++;
                count++;
            }
...
PUBLIC void HText_endAnchor ARGS1(HText*, text) {
    ...
    if (tattr) {
        ...
        if (anchorPatchIdx >= ANCHOR_LINES_SPAN) {
            fprintf(stderr, "Warning: certain anchor span more lines than the software can deal "
                            "with (lower part of the lines have no links).\n");
        }
 ...
 */
PUBLIC void HText_dump ARGS1(HText*, text) { fprintf(stderr, "HText: Dump called\n"); }
...
PUBLIC BOOL HText_selectAnchor ARGS2(HText*, text, HTChildAnchor*, anchor) {
    ...
    if (!a) {
        if (TRACE)
            fprintf(stderr, "HText: No such anchor in this text!\n");
        return NO;
    }
    {
        int l = 1;
        if (1)
            fprintf(stderr, "HText: Selecting anchor [%d] at character %d, line %d\n", a->number,
                    a->start, l);
    }
    return YES;
#endif
 ...
 */
PUBLIC HTStyle* HText_selectionStyle ARGS2(HText*, me, HTStyleSheet*, sheet) {
...
TFStruct* html_setUpTFStruct(VObj* self, char* text)
{
    ...
    if (!tf) {
        ...
        tf->firstp = tf->lastp = tf->offsetp = tf->currentp = NULL;
        ...
        tf->w = 0;
        tf->xUL = tf->yUL = tf->xLR = tf->yLR = tf->width = tf->height = 0;
...
TFStruct* html_updateTFStruct(VObj* self, char* address)
{
    ...
    if (newWidth > 2 && newHeight > 2) {
        ...
        if (!(htext = html_loadDocument(self, address, &title, &simpleAddress, &anchorSearch))) {
            return NULL;
        }
        tf = htext->tfstruct;
    ...
    if (status == 0) {
        ...
        */
        ...
        if (loadRetStatus) {
            ...
            if (!htext) {
                if (verbose)
                    fprintf(stderr, "html: Failed to access ``%s''\n", *simpleAddress);
            ...
            */
            ...
            status = 2;
            if (verbose)
                fprintf(stderr, "html: Accessed ``%s''\n", *simpleAddress);
        } else {
            if (verbose)
                fprintf(stderr, "html: Failed to access ``%s''\n", *simpleAddress);
            Vfree(GET__memoryGroup(self), tf);
        ...
        if (htext)
            if (htext->tfstruct)
                if (htext->tfstruct->self == self)
                    if (htext->simpleAddress) {
                        if (verbose)
                            fprintf(stderr, "html: FTBA %s, %s\n", htext->simpleAddress, address);
                        if (!strcmp(htext->simpleAddress, address)) {
                            return list->object;
                        }
                    }
    }
...
int setHTMLStyle() {
    ...
    if (!obj) {
        if (verbose)
            fprintf(stderr, "object ``res.HTMLStyle'' not found. Using default style.\n");
        returnPackets(2);
        return 1;
    }

    if (verbose)
        fprintf(stderr, "Processing HTML Style sheet\n");
    ...
    for (; attrp; attrp = attrp->next) {
        ...
        if (pk) {
            ...
            if (styleList[0] != '\0') {
                if (verbose)
                    fprintf(stderr, "tagName=%s, styleList=``%s''\n", tagName, styleList);
                sscanf(styleList, "%s %s %d %d %d %d %d %d %d", font, align, &wrap, &free, &first,
    ...
    if (fp) {
        ...
        for (;;) {
            if (input_pointer >= input_limit) {
                tempFileStat = NETREAD(input_file_number, input_buffer, INPUT_BUFFER_SIZE);
                if (tempFileStat <= 0) {
                    if (tempFileStat < 0) {
                        if (TRACE)
                            fprintf(stderr, "HTJustRead: File read error %d\n", tempFileStat);
                    }
                    break;
                }
