...
Packet* getlistitem(Attr* attrp)
{
    ...
    printf("\n");
    */
    for (attrp2 = (Attr*)((Packet*)(attrp->val))->info.a; attrp2; attrp2 = attrp2->next) {
        if (attrp2->id == i) {
            if (attrp2)
                return (Packet*)attrp2->val;
            return NULL;
        }
    }
    return NULL;
...
Attr* makeArgAttr(VObj* self)
{
    ...
    if (!argAttr) {
        ...
        if (!argAttr) {
            fprintf(stderr, "internal error: failed to make argAttr.\n");
            return NULL;
        }
        argPk = makePacket();
    ...
    while (pcode < pcode_end) {
        ...
        if (code & 0xe0000000) {
            ...
            case CODE_PUSH_SET2 << 16: {
                ...
                if (stackExecIdx > execStackSizeCheck) {
                    if (!incrementExecStack())
                        return 0;
                    fprintf(stderr, "warning: execution stack overflowed.\n");
                } else {
            ...
            case CODE_CALL2 << 16: {
                ...
                */
                ...
                if (allmhp.bits[funcid % allmhp.bitsSize] & (1UL << (funcid % sizeof(long)))) {
                    ...
                    entry = &(cip->mht->entries[funcid % cip->mht->size]);
                ...
                }
                {
                    ...
                    if ((entry = objID2Obj->get(objID2Obj, storeIdent(saveString(oname))))) {
                        ...
                        fprintf(stderr, "obj=%s: unknown call: %s()\n", GET_name(self),
                                (char*)symID2Str->get(symID2Str, funcid)->val);
                    }
                }
            doneCall:
            ...
            case CODE_CALL2_C << 16: {
                ...
                func(self, &reg1, argc, &execStack[stackExecIdx - argc + 1]);
                ...
                *varVectorp = (Attr**)malloc(sizeof(Attr*) * (size_t)varCount);
                while (varCount--) {
                    ...
                    */
                     ...
                     */
                    attrp = NULL;
                    for (attrp2 = varlist; attrp2; attrp2 = attrp2->next) {
                        if (attrp2->id == varID) {
                            attrp = attrp2;
                            break;
                        }
            ...
            case CODE_DIV_LISTC:
                if (reg2.canFree & PK_CANFREE_STR)
                    free(reg2.info.s);
                reg2.info.i = getlistcount((*varVectorp)[(*pcode++).i]);
                reg2.type = PKT_INT;
                reg2.canFree = 0;
                biOp(CODE_DIV, &reg1, &reg2);
                continue;
...
int setVariable(Attr* varlist, char* name, char* valp) { return 0; }
