...
#define EVENT_MASK_ABSOLUTE Button1MotionMask | StructureNotifyMask | ExposureMask
...
char* eventStr[] = {"unknown event 0", "unknown event 1", "KeyPress",         "KeyRelease",
...
void violaProcessEvent(e) XEvent* e;
{
    ...
    if (hentry) {
        ...
        if (obj && validObjectP(obj) /*XXX probably unnecessary now */) {
            VCurrentObj = obj;
            if (flag_window_tracking && VCurrentObj != VPrevObj) {
...
void violaIdleEvent() {
    ...
    if (firstTimeInfo) {
        ...
        timeout.tv_sec = firstTimeInfo->time.tv_sec - timenow.tv_sec;
        timeout.tv_usec = firstTimeInfo->time.tv_usec - timenow.tv_usec;
...
int tool;
{
    ...
    */
    ...
    case MotionNotify:
        handle_MotionNotify(ep, &dragObj, tool, &resize_corner, &mouseDown, &from_x, &from_y);
        break;
    ...
    case MapNotify: {
        ...
        if (reparented_obj) {
            GLDrawRubberFrame(reparented_obj, GET_x(reparented_obj) + 1, GET_y(reparented_obj) + 1,
                              GET_x(reparented_obj) + GET_width(reparented_obj) - 1,
                              GET_y(reparented_obj) + GET_height(reparented_obj) - 1);
            reparented_obj = NULL;
        }
        break;
    }
    ...
    case SelectionRequest:
        if (xselectionObj) {
            ...
            callMeth(xselectionObj, result, 0, NULL, STR_getSelection);
...

{
    if (VCurrentObj == NULL)
        return 0;
    if (ep->detail == NotifyVirtual || ep->detail == NotifyNonlinearVirtual)
        return 0;
    ...
    if ((tool != ACTION_TOOL) && (*mouseDown == 0)) {
        GLDrawRubberFrame(VCurrentObj, GET_x(VCurrentObj) + 1, GET_y(VCurrentObj) + 1,
                          GET_x(VCurrentObj) + GET_width(VCurrentObj) - 1,
                          GET_y(VCurrentObj) + GET_height(VCurrentObj) - 1);
    } else {
        ...
        *dragObjp = VCurrentObj;
        ...
        if (stack = ObjStack(*dragObjp)) {
            ...
            GLDrawRubberFrame(*dragObjp, GET_x(*dragObjp) + 1, GET_y(*dragObjp) + 1,
                              GET_x(*dragObjp) + GET_width(*dragObjp) - 1,
                              GET_y(*dragObjp) + GET_height(*dragObjp) - 1);
            ...
            *mouseDown = 1;
            GLQueryMouse(rootWindow, &root_x, &root_y, &(mouse.x), &(mouse.y));
...
int* mouseDown;
{
    ...
    case REPARENT_TOOL: /* release */
        if (*dragObjp) {
            ...
            "Sorry, X11R4 does not allow visible inferiors to InputOnly windows.\n");
                                                            ...
                                                            callMethod(*dragObjp, "set", result,
            "parent", result2, NULL);

                                                            callMethod(*dragObjp, "set", result,
            "visible", packetConst_1, NULL);
                                                    }
                                            reparented_obj = *dragObjp;
                                    } else {
                                            MERROR(*dragObjp,
                                                    "reparent tool can't figure out the new
            parent\n");
                                    }
            */
        }
    ...
    case RESIZE_TOOL: /* release */
        ...
        if (*dragObjp) {
            ...
            sendTokenMessageAndInts(*dragObjp, STR_config, intBuff, 4);
...
int *from_x, *from_y;
{
    ...
    case REPARENT_TOOL:
        ...
        if (*mouseDown && *dragObjp) {
            ...
            if (GET__parent(*dragObjp))
                GLUpdatePosition(0, w, GET_x(*dragObjp), GET_y(*dragObjp));
            else
                GLUpdatePosition(1, w, GET_x(*dragObjp), GET_y(*dragObjp));
    ...
    case RESIZE_TOOL: /* motion */
        ...
        if (*mouseDown && *dragObjp) {
            ...
            if (*dragObjp != findWindowObject(w))
                return 0;
...
handle_ExposeNotify(ep) XExposeEvent* ep;
{
    VObj* obj = findWindowObject(ep->window);
...
int resize_corner;
{
    if (GET_width(self) < 1) {
        SET_width(self, 1);
        if ((resize_corner == RC_LEFT) || (resize_corner == RC_UPPER_LEFT) ||
            (resize_corner == RC_LOWER_LEFT))
        ...
        SET_height(self, 1);
        if ((resize_corner == RC_UPPER) || (resize_corner == RC_UPPER_RIGHT) ||
            (resize_corner == RC_UPPER_LEFT))
...
Packet* argv;
{
     ...
     */
    if (!(new = (TimeInfo*)malloc(sizeof(struct TimeInfo))))
        return 0;
