...
BinaryOpInfo binaryOpInfo[] = {{CODE_PLUS_INT, CODE_PLUS_CHAR, CODE_PLUS_FLOAT, CODE_PLUS_STR,
                                CODE_PLUS_REF, CODE_PLUS_LISTC, CODE_PLUS_LIST, CODE_PLUS_POP},
                               {CODE_MINUS_INT, CODE_MINUS_CHAR, CODE_MINUS_FLOAT, CODE_MINUS_STR,
                                CODE_MINUS_REF, CODE_MINUS_LISTC, CODE_MINUS_LIST, CODE_MINUS_POP},
                               {CODE_MOD_INT, CODE_MOD_CHAR, CODE_MOD_FLOAT, CODE_MOD_STR,
                                CODE_MOD_REF, CODE_MOD_LISTC, CODE_MOD_LIST, CODE_MOD_POP},
                               {CODE_MULT_INT, CODE_MULT_CHAR, CODE_MULT_FLOAT, CODE_MULT_STR,
                                CODE_MULT_REF, CODE_MULT_LISTC, CODE_MULT_LIST, CODE_MULT_POP},
                               {CODE_DIV_INT, CODE_DIV_CHAR, CODE_DIV_FLOAT, CODE_DIV_STR,
                                CODE_DIV_REF, CODE_DIV_LISTC, CODE_DIV_LIST, CODE_DIV_POP},
                               {CODE_EQ_INT, CODE_EQ_CHAR, CODE_EQ_FLOAT, CODE_EQ_STR, CODE_EQ_REF,
                                CODE_EQ_LISTC, CODE_EQ_LIST, CODE_EQ_POP},
                               {CODE_NE_INT, CODE_NE_CHAR, CODE_NE_FLOAT, CODE_NE_STR, CODE_NE_REF,
                                CODE_NE_LISTC, CODE_NE_LIST, CODE_NE_POP},
                               {CODE_LT_INT, CODE_LT_CHAR, CODE_LT_FLOAT, CODE_LT_STR, CODE_LT_REF,
                                CODE_LT_LISTC, CODE_LT_LIST, CODE_LT_POP},
                               {CODE_LE_INT, CODE_LE_CHAR, CODE_LE_FLOAT, CODE_LE_STR, CODE_LE_REF,
                                CODE_LE_LISTC, CODE_LE_LIST, CODE_LE_POP},
                               {CODE_GT_INT, CODE_GT_CHAR, CODE_GT_FLOAT, CODE_GT_STR, CODE_GT_REF,
                                CODE_GT_LISTC, CODE_GT_LIST, CODE_GT_POP},
                               {CODE_GE_INT, CODE_GE_CHAR, CODE_GE_FLOAT, CODE_GE_STR, CODE_GE_REF,
                                CODE_GE_LISTC, CODE_GE_LIST, CODE_GE_POP},
                               {CODE_AND_INT, CODE_AND_CHAR, CODE_AND_FLOAT, CODE_AND_STR,
                                CODE_AND_REF, CODE_AND_LISTC, CODE_AND_LIST, CODE_AND_POP},
                               {CODE_OR_INT, CODE_OR_CHAR, CODE_OR_FLOAT, CODE_OR_STR, CODE_OR_REF,
                                CODE_OR_LISTC, CODE_OR_LIST, CODE_OR_POP}};
...
void regListRef(ast) AST* ast;
{
    ...
    */
    ...
    for (; lfap < lfap_end; lfap++)
        if (lfap->ast->attribute.info.i == id)
            return;
...
int* pc;
{
    ...
    for (i = 0; i < listRefArrayIdx; i++) {
        ...
        */
        ...
        for (j = 0; j < varArrayIdx; j++, refArrayp++) {
            if (refArrayp->flag) {
                if (refArrayp->ast->type == AST_REF && refArrayp->ast->attribute.info.i == id) {
    ...
    /* reset flags */
    for (vap = varArray, vap_end = &varArray[varArrayIdx]; vap < vap_end; vap++)
        vap->flag = 1;
    /*
...
int* pc;
{
    ...
    */
    ...
    i = 0;
    for (astp = ast->children->next; astp; astp = astp->next) {
        for (castp = astp->children->children; castp; castp = castp->next) {
...
int* pc;
{
    ...
    case AST_MOVTO_REF:
        ...
        */
        IEMIT(0xe0000000 | CODE_MOVTO_REF2 << 16 | ast->attribute.info.i);
        break;
        ...
        if (ast->children) {
            ...
            if (slotName) {
                ...
                if (slotSymID) {
                    ...
                    IEMIT(slotSymID);
                    */
                    XEMIT(0xe0000000 | CODE_GET2 << 16 | slotSymID);
                }
            } else {
                fprintf(stderr, "internal error: GET(): failed to get symID for argument.\n");
            }
        } else {
            fprintf(stderr, "error: GET() must have exactly one argument.\n");
        }
        break;
        ...
        if (ast->children) {
            ...
            if (slotName) {
                ...
                if (slotSymID) {
                    ...
                    IEMIT(slotSymID);
                    */
                    XEMIT(0xe0000000 | CODE_PUSH_SET2 << 16 | slotSymID);
                }
            } else {
                fprintf(stderr, "internal error: GET(): failed to get symID for argument.\n");
            }
    ...
    case AST_LIST_REF:
        ...
        IEMIT(ast->attribute.info.i);
        */
        IEMIT(0xe0000000 | CODE_LIST_REF2 << 16 | ast->attribute.info.i);
        break;
    ...
    default:
        fprintf(stderr, "internal error: unknown ast type = %d:\n", ast->type);
        fprintf(stderr, "-----------------\n");
        printAST(ast, 0);
        fprintf(stderr, "-----------------\n");
        break;
