...
VObj* SGMLBuildDoc(char* srcp, char* url, VObj* parent, char* name, int width, char* anchor)
{
    ...
    fp = fopen(url, "r");
    if (!fp) {
        fprintf(stderr, "SGMLBuildDoc(): failed to open file '%s'\n", url);
        return NULL;
    }
    ...
    docObj = SGMLBuild(dmi, tagMappingInfo, tagName, localizingTagIDMap, parent, width, y, &h, 0);
    if (!docObj) {
        if (sgml_verbose)
...
void loadTemplateObjectNMethod(SGMLTagMappingInfo* tmi)
{
    ...
    */
    ...
    if (!tmi->template) {
        fprintf(stderr, "loadTemplateObjectNMethod: can't get template object '%s'\n", tmi->type);
    }
    if (tmi->template) {
...
VObj* SGMLBuild(SGMLDocMappingInfo* dmi, SGMLTagMappingInfo* tagMappingInfo, char* tagName, int* localizingTagIDMap, VObj* parent, int width, int y, int* h, int PinlineP)
{
    ...
    if (!tmi) {
        fprintf(stderr, "SGMLBuild: Don't know about tag '%s'\n", tagName);
        --dataBuffIdxStackIdx;
        return NULL;
    }
    ...
    */
    ...
    if (!template) {
        ...
        template = tmi->template;
        if (!template) {
            fprintf(stderr, "SGMLBuild: Failed to get template object\n");
            --dataBuffIdxStackIdx;
            return NULL;
        }
    }
    ...
    if (!obj) {
        ...
        fprintf(stderr, "SGMLBuild: failed to clone template object '%s'\n", GET_name(template));
        fprintf(stderr, "SGMLBuild: STR_clone=%d\n", STR_clone);
    ...
    dtd = dmi->DTDName;
    SET__varList(obj, setVariable_id_STR(GET__varList(obj), STR_dtd, dtd, 0));

    /* ick ick ick etc */
    SET__eventMask(obj, GET__eventMask(obj) & ~(EnterWindowMask | LeaveWindowMask));
    ...
    if (obj && elemAttrStackCount) {
        ...
        for (i = 0; i < elemAttrStackCount; i++) {
            ...
            if (!strcmp(name, "NAME")) {
                ...
                if (elemAttrStack_val[i]) {
                    sendMessage1N2str(obj, "AA", name, elemAttrStack_val[i]);
                } else {
    ...
    *h = 0;
        ...
        case '(': {
            ...
            if (obj && ctmi)
                if (tmi->flushAlwaysP || ctmi->flushOnSubTagP) {
                    ...
                    size = dataBuffIdx - dataBuffIdxStack[dataBuffIdxStackIdx];
                    cp = (char*)malloc(sizeof(char) * (size + 1));
                    if (size) {
                        strncpy(cp, &dataBuff[dataBuffIdxStack[dataBuffIdxStackIdx]], size);
                    }
                    cp[size] = '\0';
                    if (GET_label(obj))
                        free(GET_label(obj));
                    ...
                    */
                    ...
                    if (obj)
                        if (sgml_verbose)
                            fprintf(stderr, "exiting %s. obj='%s' span=%d *h=%d\n", "tag",
                                    GET_name(obj), span, *h);
            ...
            if (obj) {
                if (insert_obj) {
                    ...
                    subObj = SGMLBuild(dmi, tagMappingInfo, subTagName, localizingTagIDMap,
                                       _insert_obj, _width, 0, &span, tmi->inlineP);

                    olist = GET__children(obj);
                    SET__children(obj, appendObjToList(olist, _insert_obj));

                    if (subObj)
                        if (!tmi->inlineP) {
                            olist = GET__children(_insert_obj);
                            *h += span;
                            SET__children(_insert_obj, appendObjToList(olist, subObj));
                        }
                    ...
                    subObj = SGMLBuild(dmi, tagMappingInfo, subTagName, localizingTagIDMap, obj,
                                       width, *h, &span, tmi->inlineP);
                    if (subObj)
                        if (!tmi->inlineP) {
                            VObjList* olist = GET__children(obj);

                            *h += span;
                            SET__children(obj, appendObjToList(olist, subObj));
                        }
                }
            }
        } break;
        case ')': {
            ...
            if (obj) {
                if (tmi->inlineP) {
                    ...
                    if (GET_label(obj))
                        free(GET_label(obj));
                    SET_label(obj, &dataBuff[dataBuffIdxStack[dataBuffIdxStackIdx]]);
                    ...
                    */
                    /*
                    printf("obj=%s lidx=%d idx=%d DATA {%s}\n",
                    GET_name(obj), dataBuffIdxStack[dataBuffIdxStackIdx], dataBuffIdx,
                    GET_label(obj));
                    */
                    /* in case obj needs this information */
                    SGMLBuildDoc_span = *h;
    ...
    if (sgml_verbose)
        fprintf(stderr, "exiting %s (end). obj='%s'\n", "tag", GET_name(obj));
...
VObj* SGMLBuildDoc_B(char* srcp, char* url, VObj* parent, char* name, int width, char* anchor)
{
    ...
    */
    ...
    for (i = 0; i < tagDictCount; i++) {
        ...
        if (!tmi->template) {
            if (sgml_verbose)
                fprintf(stderr, "SGMLBuild_B: preloading object '%s'\n", tmi->type);
            ...
            tmi->template = PkInfo2Obj(&argv[0]);
            if (!tmi->template) {
                fprintf(stderr, "SGMLBuild_B: can't get template object '%s'\n", tmi->type);
            }
    ...
    docObj = SGMLBuild_B(dmi, tagMappingInfo, tagID, localizingTagIDMap, parent, width, y, &h, 0);
    if (!docObj) {
        if (sgml_verbose)
            fprintf(stdout, "SGMLBuildDoc_B: docObj = NULL\n");
        fclose(fp);
...
VObj* SGMLBuild_B(SGMLDocMappingInfo* dmi, SGMLTagMappingInfo* tagMappingInfo, int tagID, int* localizingTagIDMap, VObj* parent, int width, int y, int* h, int PinlineP)
{
    ...
    if (!tmi) {
        fprintf(stderr, "SGMLBuild_B: Don't know about tag (id=%d)\n", tagID);
        --dataBuffIdxStackIdx;
        return NULL;
    }
    /*
    ...
    */
    template = tmi->template;
    if (!template) {
        fprintf(stderr, "SGMLBuild_B: Failed to get template object\n");
        --dataBuffIdxStackIdx;
        return NULL;
    }
    /* clone template object
    ...
    if (!obj) {
        ...
        fprintf(stderr, "SGMLBuild_B: failed to clone template object '%s'\n", GET_name(template));
    ...
    */
    /* ick ick ick etc */
    SET__eventMask(obj,
                   (GET__eventMask(obj) & ~(EnterWindowMask | LeaveWindowMask)) | MotionNotify);
        ...
        case TOKEN_TAG: {
            ...
            if (obj && ctmi)
                if (tmi->flushAlwaysP || ctmi->flushOnSubTagP) {
                    ...
                    size = dataBuffIdx - dataBuffIdxStack[dataBuffIdxStackIdx];
                    if (size < 0) {
                        fprintf(stderr, "sgml parsing error occured.\n");
                        size = 0;
                    }
                    if (size) {
                        cp = (char*)malloc(sizeof(char) * (size + 1));
            ...
            if (obj) {
                if (insert_obj) {
                    ...
                    subObj = SGMLBuild_B(dmi, tagMappingInfo, tagID, localizingTagIDMap,
                                         _insert_obj, _width, 0, &span, tmi->inlineP);
                    ...
                    subObj = SGMLBuild_B(dmi, tagMappingInfo, tagID, localizingTagIDMap, obj, width,
                                         *h, &span, tmi->inlineP);
                    if (subObj)
                        if (!tmi->inlineP) {
                            VObjList* olist = GET__children(obj);
...
int getSGMLStyle(char* dtd, char* tagName, char* attrName, Packet* result)
{
    ...
    if (attrName) {
        ...
        if (!tmi->returnCache) {
            ...
            packet0->canFree = packet1->canFree = packet2->canFree = packet3->canFree =
                PK_CANFREE_NOT;
            packet0->type = packet1->type = packet2->type = packet3->type = PKT_INT;
...
SGMLDocMappingInfo* setSGMLStylesheet(char* dtd, char* stylesheet)
{
    ...
    if (i >= MAX_SGML_FORMS) {
        fprintf(stderr, "setSGMLStylesheet: exceeded MAX_SGML_FORMS. failed.\n");
        return 0;
    }
    dmi = &SGMLForms[i];
    ...
    tagMappingInfo = dmi->tagMap =
        (SGMLTagMappingInfo*)malloc(sizeof(struct SGMLTagMappingInfo) * (tmiIdxLimit + 1));
    if (!tagMappingInfo) {
    ...
    for (cp = retStrp; *cp; cp++) {
        ...
        if (*cp == '\n') {
            ...
            if (buff[0] != '#' && !isspace(buff[0])) {
                ...
                sscanf(buff, "%s %s %d %d %d %d %d %d %d %d %d", tagName, type, &inlineP,
                       &notifyOnCreationP, &flushAlwaysP, &flushOnSubTagP, &hpaneP, &top, &bttm,
                       &left, &right);
                /*
