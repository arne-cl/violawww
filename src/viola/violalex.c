...
int reportError(void) {
    ...
    if (yyscriptcontext == SCRIPT_ORIGIN_OBJ_SCRIPT) {
        fprintf(stderr, "error at line %d... (obj=%s, in script)\n", lineno, yyobjcontext);
    } else if (yyscriptcontext == SCRIPT_ORIGIN_CLASS_SCRIPT) {
        fprintf(stderr, "error at line %d... (obj=%s, in classScript)\n", lineno, yyobjcontext);
    } else {
        fprintf(stderr, "error at line %d... (obj=%s, in eval)\n", lineno, yyobjcontext);
...
int yyerror(char* s)
{
    ...
    sprintf(buf, "Parse error near line %d, character %d.", lineno, chars);
    lexHandleError(buf);
...
int init_scanutils() {
    ...
    if (verbose)
        printf("Built trie with %d levels (%d bytes.)\n", numtries, numtries * 128 * 4);
...
static int grabstring() {
    ...
    endstr:
        ...
        if (total_len)
            product = realloc_strcat(product, buf, total_len, i);
        else
            product = alloc_stringn(buf, i);
