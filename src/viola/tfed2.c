    ...
    if (theEditLN->length > 0) {
        if (tf->highLiteFrom_cx != -1) {
            ...
            tf->current_col_sticky = tf->current_col =
                (tf->highLiteFrom_cx < tf->highLiteTo_cx ? tf->highLiteFrom_cx : tf->highLiteTo_cx);
            tf->current_row =
                (tf->highLiteFrom_cy < tf->highLiteTo_cy ? tf->highLiteFrom_cy : tf->highLiteTo_cy);

            /* move cursor pixel coords to the upper-left edge */
            tf->csr_py = tf->yUL;
            currentp = tf->offsetp;
            for (i = tf->screen_row_offset; i < tf->current_row; i++) {
                tf->csr_py += currentp->maxFontHeight * currentp->breakc;
            ...
            for (i = 0; i < tf->current_col; i++) {
                ...
                if (TFCFlags(tf->currentp->linep + i) & MASK_WRAP) {
                    tf->csr_px = tf->xUL;
                }
            }

            rangeOperation(tf, tf->highLiteFrom_cx, tf->highLiteFrom_cy, tf->highLiteTo_cx,
                           tf->highLiteTo_cy, 0 /*draw*/, -999 /*doesn`t matter?*/,
                           RANGEMODE_DELETE);
...
int scrollLineBackward(TFStruct* tf, int delta, int col)
{
    ...
    while (hasMore) {
        ...
        if (TFWINDOW) {
            XCopyArea(display, TFWINDOW, TFWINDOW, gc_copy0, px + pwidth, py,
                      tf->width - px - pwidth + 1, currentp->maxFontHeight, px, py);
        }
            ...
            if (!TFCChar(tfcp)) {
                ...
                if (TFWINDOW) {
                    ...
                    fprintf(stdout, "tfed: 14 XClearArea w=%x %d %d %d %d\n", TFWINDOW,
                            px + TFCWidth(tfcp), py, tf->xLR - px, currentp->maxFontHeight);
#endif

                    XClearArea(display, TFWINDOW, px + TFCWidth(tfcp), py, tf->xLR - px,
                ...
                if (diff_breakc > 0) {
                    ...
                    tf->currentp->breakc = currentp->breakc;
                    tf->currentp->maxFontHeight = currentp->maxFontHeight;

                    currentp->breakc = orig_breakc;
                    scrollUpLowerPart(tf, tf->current_row - tf->screen_row_offset,
        ...
        if (TFWINDOW) {
#ifdef VERBOSE_SCROLLINEBACKWARD
            fprintf(stdout, "tfed: 15 XClearArea w=%x %d %d %d %d %d\n", TFWINDOW, patchstart_px,
                    py, tf->xLR - patchstart_px, pwidth, currentp->maxFontHeight);
#endif

            XClearArea(display, TFWINDOW, patchstart_px, py, tf->xLR, currentp->maxFontHeight,
                       False);
        }
#ifdef VERBOSE_SCROLLINEBACKWARD
