...
SlotInfo cl_field_NCSlots[] = {0};
SlotInfo cl_field_NPSlots[] = {{STR_content, PTRS | SLOT_RW, (long)""},
                               {STR__content, /* internalized format */
                                LONG, 0},
                               {STR_content2, PTRS | SLOT_RW, (long)""},
                               {STR__content2, /* internalized format */
                                LONG, 0},
                               {STR_label, PTRS | SLOT_RW, (long)""},
                               {STR__label, /* internalized format */
                                LONG, 0},
                               {STR_border, LONG | SLOT_RW, BORDER_FIELD},
                               {STR_x, LONG | SLOT_RW, 0},
                               {STR_y, LONG | SLOT_RW, 0},
                               {STR_width, LONG | SLOT_RW, 1},
                               {STR_height, LONG | SLOT_RW, 1},
                               {STR_minWidth, LONG | SLOT_RW, 1},
                               {STR_minHeight, LONG | SLOT_RW, 1},
                               {STR_maxWidth, LONG | SLOT_RW, 9999},
                               {STR_maxHeight, LONG | SLOT_RW, 9999},
                               {STR_gapH, LONG | SLOT_RW, 0},
                               {STR_gapV, LONG | SLOT_RW, 0},
                               {STR_window, /* probably should hide this (internal) */
                                LONG, 0},
                               {STR_shownPositionH, LONG | SLOT_R, 0},
                               {STR_shownPositionV, LONG | SLOT_R, 0},
                               {STR_shownSizeH, LONG | SLOT_R, 0},
                               {STR_shownSizeV, LONG | SLOT_R, 0},
                               {STR_shownDepend, PTRS | SLOT_RW, (long)""},
                               {STR__shownDepend, OBJL, 0},
                               {STR_shownNotify, PTRS | SLOT_RW, (long)""},
                               {STR__shownNotify, OBJL, 0},
                               {STR_lock, LONG, 0},
                               {STR_visible, LONG, 0},
                               {STR__colorInfo, RGBV, 0},
                               {
                                   STR_BDColor,      /* border color */
                                   PTRS | SLOT_RW, 0 /*(long)"black"*/
                               },
                               {
                               ...
                               {STR__eventMask, LONG,
                                (long)KeyPressMask | KeyReleaseMask | ButtonPressMask |
                                    ButtonReleaseMask | EnterWindowMask | LeaveWindowMask |
                                    PointerMotionHintMask | Button1MotionMask | Button2MotionMask |
                                    Button3MotionMask | StructureNotifyMask | ExposureMask},
                               {STR_eventMask, PTRS | SLOT_RW, (long)""},
                               {0}};
SlotInfo cl_field_CSlots[] = {{STR_class, PTRS | SLOT_RW, (long)"field"},
                              {
                                  STR_classScript,
                                  PTRS,
                                  (long)"\n\
	...
	",
                              ...
                              {0}};
SlotInfo cl_field_PSlots[] = {{STR__classInfo, CLSI, (long)&class_field}, {0}};

SlotInfo* slots_field[] = {(SlotInfo*)cl_field_NCSlots, (SlotInfo*)cl_field_NPSlots,
                           (SlotInfo*)cl_field_CSlots, (SlotInfo*)cl_field_PSlots};

MethodInfo meths_field[] = {
    /* local methods */
    {STR_canvalize, meth_field_canvalize},
    {
        STR_clean,
        meth_field_clean,
    },
    ...
    {STR_clearWindow, meth_field_clearWindow},
    {STR_clone, meth_field_clone},
    {STR_clone2, meth_field_clone2},
    {STR_config, meth_field_config},
    {
        STR_copyArea,
        meth_field_copyArea,
    },
    {STR_cycleColors, meth_field_cycleColors},
    {STR_deepObjectListSend, meth_field_deepObjectListSend},
    {
        STR_drawFillOval,
    ...
    {STR_geta, meth_field_get},
    {STR_iconName, meth_field_iconName},
    {STR_initialize, meth_field_initialize},
    {
        STR_invertFillOval,
    ...
    {STR_invertText, meth_field_invertText},
    {STR_lower, meth_field_lower},
    {STR_mouse, meth_field_mouse},
    {STR_mouseLocal, meth_field_mouseLocal},
    {STR_mouseButton, meth_field_mouseButton},
    {STR_mouseX, meth_field_mouseX},
    {STR_mouseY, meth_field_mouseY},
    {STR_objectListAppend, meth_field_objectListAppend},
    {STR_objectListCount, meth_field_objectListCount},
    {STR_objectListDelete, meth_field_objectListDelete},
    {STR_objectListPrepend, meth_field_objectListPrepend},
    {STR_objectListSend, meth_field_objectListSend},
    {STR_raise, meth_field_raise},
    {
        STR_randomizeArea,
        meth_field_randomizeArea,
    },
    {STR_render, meth_field_render},
    {STR_smudge, meth_field_smudge},
    {STR_seta, meth_field_set},
    {STR_windowName, meth_field_windowName},
    {STR_windowPosition, meth_field_windowPosition},
    {0}};

ClassInfo class_field = {
    helper_field_get, helper_field_set, slots_field, /* class slot information	*/
...
long meth_field_clearArea(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (GET_window(self)) {
        if (GET__classInfo(self) != &class_glass) {
            GLPaintFillRect(GET_window(self), gc_bg, x0, y0, x1, y1);
        }
        return 1;
    }
    return 0;
...
long meth_field_clone(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (!meth_field_clone2(self, result, argc, argv))
        return 0;
...
long meth_field_copyArea(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (GET_window(self)) { /*XXX*/
        XCopyArea(display, GET_window(self), GET_window(self), gc_copy, fromx, fromy, width, height,
                  tox, toy);
        return 1;
    }
    return 0;
...
long meth_field_fillArc(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (GET_window(self)) {
        if (GET__classInfo(self) != &class_glass) {
            GLPrepareObjColor(self);
            GLPaintFillArc(GET_window(self), gc_fg, x0, y0, x1, y1, PkInfo2Int(&argv[4]),
                           PkInfo2Int(&argv[5]));
            return 1;
        }
    }
...
long meth_field_eraseText(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (GET_window(self) && GET_visible(self)) {
        if (GET__classInfo(self) != &class_glass) {
            GLPrepareObjColor(self);
            GLEraseText(GET_window(self), fontID, x0, y0, PkInfo2Str(&argv[3]));
            return 1;
        }
    }
    return 0;
...
long meth_field_get(VObj* self, Packet* result, int argc, Packet argv[]) {
    return helper_field_get(self, result, argc, argv, (int)getIdent(PkInfo2Str(argv)));
}
...
long meth_field_initialize(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (cp)
        if (*cp)
            SET__shownDepend(self, strOListToOList(cp));

    cp = GET_shownNotify(self);
    if (cp)
        if (*cp)
            SET__shownNotify(self, strOListToOList(cp));
...
long meth_field_invertText(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (GET_window(self)) {
        GLPrepareObjColor(self);
        GLInvertText(GET_window(self), fontID, x0, y0, PkInfo2Str(&argv[3]));
        return 1;
    }
    return 0;
}
        ...
        if (olist) {
            ...
            */
            ...
            if (GET_window(self)) {
                ...
                fprintf(stderr, "obj=%s: lower() failed, no window.\n", GET_name(self));
                result->info.i = 0;
                return 0;
            }
        }
...
long meth_field_mouseButton(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (mouseButtonPressedState & MOUSE_BUTTON_1)
        state[0] = '1';
    if (mouseButtonPressedState & MOUSE_BUTTON_2)
        state[1] = '1';
    if (mouseButtonPressedState & MOUSE_BUTTON_3)
        state[2] = '1';
    if (mouseButtonPressedState & MOUSE_BUTTON_4)
        state[3] = '1';
    if (mouseButtonPressedState & MOUSE_BUTTON_5)
        state[4] = '1';
    result->type = PKT_STR;
    result->info.s = state;
    result->canFree = PK_CANFREE_STR;
...
long meth_field_objectListPrepend(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (!strcmp(listName, "shownDepend")) {
        ...
        olist = GET__shownDepend(self);
        if (obj) {
            SET__shownDepend(self, prependVObjListNode(olist, obj));
            result->info.o = obj;
            return 1;
        }
    } else if (!strcmp(listName, "shownNotify")) {
        ...
        olist = GET__shownNotify(self);
        if (obj) {
            SET__shownNotify(self, prependVObjListNode(olist, obj));
            result->info.o = obj;
            return 1;
        }
    } else {
    ...
    if (parent) {
        ...
        if (olist) {
            ...
            if (GET_window(self)) {
                ...
                fprintf(stderr, "obj=%s: raise() failed, no window.\n", GET_name(self));
                result->info.i = 0;
                return 0;
            }
        }
...
long helper_field_set(VObj* self, Packet* result, int argc, Packet argv[], long labelID) {
    ...
    case STR_x:
        ...
        result->canFree = 0;
        if (GET_window(self))
            GLUpdatePosition(!GET__parent(self), GET_window(self), GET_x(self), GET_y(self));
        return 1;
        ...
        if (GET_window(self)) {
            /*
                                    GLUpdateGeometry(!GET__parent(self), GET_window(self),
                                            ...
                                            (unsigned int)GET_width(self), (unsigned
               int)GET_height(self));
            */
            if (GET_width(self) > 0 && GET_height(self) > 0) {
                XSelectInput(display, GET_window(self),
                             GET__eventMask(self) & ~StructureNotifyMask);
                XResizeWindow(display, GET_window(self), (unsigned int)GET_width(self),
                              (unsigned int)GET_height(self));
                XSelectInput(display, GET_window(self), GET__eventMask(self));
            }
        }
        return 1;
    ...
    case STR_content2:
        if (GET_content2(self))
            Vfree(GET__memoryGroup(self), GET_content2(self));
        result->info.s = VSaveString(GET__memoryGroup(self), PkInfo2Str(&argv[1]));
    ...
    case STR_visible: {
        ...
        */
        ...
        if (PkInfo2Int(&argv[1])) {
            ...
            if (!w) {
                GLPrepareObjColor(self);
                w = GLOpenWindow(self, GET_x(self), GET_y(self), GET_width(self), GET_height(self),
                                 (GET__classInfo(self) == &class_glass ? 1 : 0));
                if (!w) {
                    result->info.i = 0;
                    SET_window(self, 0);
                    SET_visible(self, 0);
    ...
    case STR_shownPositionSizeH: {
        ...
        if (GET_shownPositionH(self) != pos || GET_shownSizeH(self) != siz) {
            SET_shownPositionH(self, pos);
    ...
    case STR_shownPositionSizeV: {
        ...
        if (GET_shownPositionV(self) != pos || GET_shownSizeV(self) != siz) {
...
long meth_field_set(VObj* self, Packet* result, int argc, Packet argv[]) {
    return helper_field_set(self, result, argc, argv, (int)getIdent(PkInfo2Str(argv)));
}
...
long meth_field_smudge(VObj* self, Packet* result, int argc, Packet argv[]) {
    ...
    if (GET_window(self) && GET_visible(self)) {
        ...
        if (GET__classInfo(self) != &class_glass) {
            ...
            if (!thumbXPM)
                return 0;
            XCopyArea(display, thumbXPM, GET_window(self), gc_or, 0, 0, thumbXPM_width,
                      thumbXPM_height, wx - thumbXPM_width / 2, wy - thumbXPM_height / 2);
            return 1;
        }
    }
    return 0;
