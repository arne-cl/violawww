...
VObj* findNewObject(VObj* self) {
    ...
    if (FCardObj)
        if (nobj = findClickedObj(FCardObj, mouse.x, mouse.y))
            obj = nobj;
    if (BCardObj)
        if (nobj = findClickedObj(BCardObj, mouse.x, mouse.y))
            obj = nobj;
    if (stackObj)
        if (nobj = findClickedObj(stackObj, mouse.x, mouse.y))
            obj = nobj;
...
VObj findClickedObj(VObj self, int mx, int my)
{
    ...
    if (olist = ObjFieldList(self))
        if (matchObj = findClickedObjFromList(olist, mx, my))
            if (obj = findClickedObj(matchObj, mx, my))
                return (obj);
        ...
        if (olist->o) {
            ...
            do {
                if ((param = ObjParam(olist->o)) == NULL) {
                    fprintf(stderr, "findClickedObjFromList: no param attribute in object '%s'\n",
                            ObjName(olist->o));
                    return NULL;
                }
...
void renderObjectParam(self) VObj self;
{
    if (self) {
        OLIST* olist;
        Param* param = ObjParam(self);

        drawRubberFrame(self, ParamSx(param) + 1, ParamSy(param) + 1, ParamDx(param) - 1,
                        ParamDy(param) - 1);
