...
MethodInfo meths_slider[] = {
    /* local methods */
    {STR_config, meth_slider_config},
    {
        STR_geta,
        meth_slider_get,
    },
    {STR_initialize, meth_slider_initialize},
...
long meth_slider_render(VObj* self, Packet* result, int argc, Packet argv[])
{
    ...
    if (w && GET_visible(self)) {
        ...
        thumbPositionPixels =
            ((float)thumbPosition / 100.0) * (float)(paramExtent - thumbSizePixels);
        ...
        case 't': /* top to bottom */
            GLDrawScrollBarV(w, GET_height(self), thumbStartPixel, thumbEndPixel, barThickness);
            break;
        case 'b': /* bottom to top */
            GLDrawScrollBarV(w, GET_height(self), GET_height(self) - thumbEndPixel,
                             GET_height(self) - thumbStartPixel, barThickness);
            break;
        case 'l': /* left to right */
            GLDrawScrollBarH(w, GET_width(self), thumbStartPixel, thumbEndPixel, barThickness);
            break;
        case 'r': /* right to left */
            GLDrawScrollBarH(w, GET_width(self), GET_width(self) - thumbEndPixel,
                             GET_width(self) - thumbStartPixel, barThickness);
            break;
        }
        return 1;
    }
...
long meth_slider_processMouseInput(VObj* self, Packet* result, int argc, Packet argv[])
{
    ...
    if (GrabSuccess !=
        XGrabPointer(display, w, TRUE, 0, GrabModeAsync, GrabModeAsync, None, None, CurrentTime))
        return 1;
    ...
    for (;;) {
        ...
        switch (eventType(e)) {
        case ButtonRelease:
            mouseButtonPressedState &= ~(1 << (((XButtonEvent*)&e)->button));
            return 1;
        ...
        case ButtonPress:
            ...
            return 1;
        default:
            hentry = window2Obj->get(window2Obj, (long)eventWindow(e));
            if (hentry) {
                obj = (VObj*)hentry->val;
                if (obj)
                    VCurrentObj = obj;
