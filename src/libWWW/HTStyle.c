...
HTStyle* HTStyleRead(HTStyle* style, HTStream* stream) {
    ...
    NXScanf(stream, "%s%s%f%d", myTag, fontName, &style->fontSize, &gotpara);
    if (gotpara) {
        if (!style->paragraph) {
        ...
        p = style->paragraph;
        NXScanf(stream, "%f%f%f%f%hd%f%f%hd", &p->indent1st, &p->indent2nd, &p->lineHt,
                &p->descentLine, &p->alignment, &style->spaceBefore, &style->spaceAfter,
                &p->numTabs);
        if (p->tabs)
            free(p->tabs);
...
HTStyle* HTStyleWrite(HTStyle* style, NXStream* stream) {
    ...
    if (p) {
        NXPrintf(stream, "\t%f %f %f %f %d %f %f\t%d\n", p->indent1st, p->indent2nd, p->lineHt,
                 p->descentLine, p->alignment, style->spaceBefore, style->spaceAfter, p->numTabs);
...
HTStyle* HTStyleNamed ARGS2(HTStyleSheet*, self, const char*, name) {
    ...
    for (scan = self->styles; scan; scan = scan->next)
        if (0 == strcmp(scan->name, name))
            return scan;
    if (TRACE)
        fprintf(stderr, "StyleSheet: No style named `%s'\n", name);
    return 0;
}
...
HTStyle* HTStyleForRun(HTStyleSheet* self, NXRun* run) {
    ...
    if (TRACE)
        fprintf(stderr, "HTStyleForRun: Best match for style is %d out of 18\n", bestMatch);
    return best;
}
#endif
...
**	----------------------
*/
HTStyleSheet* HTStyleSheetAddStyle ARGS2(HTStyleSheet*, self, HTStyle*, style) {
...
HTStyleSheet* HTStyleSheetRead(HTStyleSheet* self, NXStream* stream) {
    ...
    if (TRACE)
        fprintf(stderr, "Stylesheet: Reading %d styles\n", numStyles);
...
HTStyleSheet* HTStyleSheetWrite(HTStyleSheet* self, NXStream* stream) {
    ...
    if (TRACE)
        fprintf(stderr, "StyleSheet: Writing %d styles\n", numStyles);
    for (style = self->styles; style; style = style->next) {
