...
PUBLIC void HTChunkPutc ARGS2(HTChunk*, ch, char, c) {
    if (ch->size >= ch->allocated) {
        ...
        ch->data =
            ch->data ? (char*)realloc(ch->data, ch->allocated) : (char*)malloc(ch->allocated);
        if (!ch->data)
...
PUBLIC void HTChunkEnsure ARGS2(HTChunk*, ch, int, needed) {
    ...
    ch->data = ch->data ? (char*)realloc(ch->data, ch->allocated) : (char*)malloc(ch->allocated);
    if (ch->data == NULL)
...
**	-----------------
*/
PUBLIC void HTChunkTerminate ARGS1(HTChunk*, ch) { HTChunkPutc(ch, (char)0); }
