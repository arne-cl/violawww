...
PUBLIC int HTAA_getUid NOARGS {
            ...
            if (NULL != (pw = getpwuid(atoi(current_prot->uid_name)))) {
                if (TRACE)
                    fprintf(stderr, "%s(%s) returned (%s:%s:%d:%d:...)\n", "HTAA_getUid: getpwuid",
                            current_prot->uid_name, pw->pw_name, pw->pw_passwd, pw->pw_uid,
                            pw->pw_gid);
                return pw->pw_uid;
            }
            ...
            if (NULL != (pw = getpwnam(current_prot->uid_name))) {
                if (TRACE)
                    fprintf(stderr, "%s(\"%s\") %s (%s:%s:%d:%d:...)\n", "HTAA_getUid: getpwnam",
                            current_prot->uid_name, "returned", pw->pw_name, pw->pw_passwd,
                            pw->pw_uid, pw->pw_gid);
                return pw->pw_uid;
            }
        }
    ...
    */
    if (NULL != (pw = getpwnam("nobody"))) {
        if (TRACE)
            fprintf(stderr, "HTAA_getUid: Uid for `nobody' is %d\n", pw->pw_uid);
        return pw->pw_uid;
    }
    /*
    ** Ok, then use default.
...
PUBLIC int HTAA_getGid NOARGS {
            ...
            if (NULL != (gr = getgrgid(atoi(current_prot->gid_name)))) {
                if (TRACE)
                    fprintf(stderr, "%s(%s) returned (%s:%s:%d:...)\n", "HTAA_getGid: getgrgid",
                            current_prot->gid_name, gr->gr_name, gr->gr_passwd, gr->gr_gid);
                return gr->gr_gid;
            }
    ...
    */
    if (NULL != (gr = getgrnam("nogroup"))) {
        if (TRACE)
            fprintf(stderr, "HTAA_getGid: Gid for `nogroup' is %d\n", gr->gr_gid);
        return gr->gr_gid;
    }
    /*
    ** Ok, then use default.
                        ...
                        if (scheme != HTAA_UNKNOWN) {
                            ...
                            if (TRACE)
                                fprintf(stderr, "%s %s `%s'\n", "HTAA_parseProtFile: valid",
                                        "authentication scheme:", HTAAScheme_name(scheme));
                        } else if (TRACE)
            ...
            if (lex_item != LEX_EOF && lex_item != LEX_REC_SEP) {
                if (TRACE)
                    fprintf(stderr, "%s %s %d (that line ignored)\n",
                            "HTAA_parseProtFile: Syntax error", "in protection setup file at line",
                            lex_line);
                do {
                    lex_item = lex(fp);
...
*/
PUBLIC HTAAProt* HTAA_getCurrentProtection NOARGS { return current_prot; }
