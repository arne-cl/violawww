...
PRIVATE HTAAUser* decompose_auth_string ARGS2(char*, authstring, HTAAScheme, scheme) {
    ...
    */
    if (scheme == HTAA_PUBKEY) {
        if (!(inet_addr = strchr(password, ':')) || (*(inet_addr++) = '\0'),
            !(timestamp = strchr(inet_addr, ':')) || (*(timestamp++) = '\0'),
            !(browsers_key = strchr(timestamp, ':')) || (*(browsers_key++) = '\0')) {
    ...
    if (TRACE) {
        if (scheme == HTAA_BASIC)
            ...
            fprintf(stderr, "decompose_auth_string: %s (%s,%s,%s,%s,%s)\n",
                    "Pubkey scheme authentication string:", username, password, inet_addr,
...
*/
PUBLIC HTAAUser* HTAA_authenticate ARGS3(HTAAScheme, scheme, char*, scheme_specifics, HTAAProt*,
                                         prot) {
    ...
    case HTAA_PUBKEY: {
        ...
        if (user &&
            ...
            (HTAA_BASIC == scheme ||
             (HTAA_checkTimeStamp(user->timestamp) && HTAA_checkInetAddress(user->inet_addr))))
            return user;
        else
            return NULL;
    } break;
