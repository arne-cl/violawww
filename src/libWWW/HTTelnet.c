...
PRIVATE int remote_session ARGS2(char*, access, char*, host) {
    ...
    enum _login_protocol {
        ...
        rlogin,
        tn3270
    } login_protocol = strcmp(access, "rlogin") == 0   ? rlogin
    ...
    if (HTSecure) {
        ...
        sprintf(command, "finger @%s | mail -s \"**telnethopper %s\" tbl@dxcern.cern.ch",
...
#ifndef TELNET_DONE
    fprintf(stderr, "Sorry, this browser was compiled without the %s access option.\n", access);
    fprintf(stderr, "\nTo access the information you must %s to %s", access, hostname);
    if (port)
        fprintf(stderr, " (port %s)", port);
    if (user)
        fprintf(stderr, " logging in with username %s", user);
    fprintf(stderr, ".\n");
    return -1;
#endif
}
