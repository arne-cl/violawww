...
PRIVATE void flush ARGS1(HTStream*, me) {
    ...
    while (read_pointer < write_pointer) {
        ...
        if (status < 0) {
            if (TRACE)
                fprintf(stderr, "HTWrite: Error: write() on socket returns %d !!!\n", status);
            return;
        }
...
PRIVATE void HTWriter_write ARGS3(HTStream*, me, const char*, s, int, l) {
    ...
    while (read_pointer < write_pointer) {
        int status = NETWRITE(me->soc, read_pointer, write_pointer - read_pointer);
        if (status < 0) {
            if (TRACE)
...
PRIVATE void HTWriter_abort ARGS2(HTStream*, me, HTError, e) { HTWriter_free(me); }
