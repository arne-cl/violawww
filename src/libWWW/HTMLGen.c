    ...
    *me->write_pointer++ = c;
    ...
    if (me->write_pointer == me->buffer + BUFFER_SIZE) {
        if (me->cleanness) {
            ...
            (*me->targetClass.put_block)(me->target, me->buffer, BUFFER_SIZE);
            me->write_pointer = me->buffer;
        }
        me->line_break = me->buffer;
    }
...
PRIVATE void HTMLGen_abort ARGS2(HTStructured*, me, HTError, e) { HTMLGen_free(me); }

PRIVATE void PlainToHTML_abort ARGS2(HTStructured*, me, HTError, e) { PlainToHTML_free(me); }
...
PUBLIC HTStream* HTPlainToHTML ARGS3(HTPresentation*, pres, HTParentAnchor*, anchor, HTStream*,
                                     sink) {
    HTStructured* me = (HTStructured*)malloc(sizeof(*me));
    if (me == NULL)
