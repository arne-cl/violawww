...
PUBLIC char* HTCacheFileName(const char* name) {
    ...
    char* result;
    result =
        (char*)malloc(strlen(HTCacheRoot) + strlen(access) + strlen(host) + strlen(path) + 6 + 1);
    if (result == NULL)
...
PUBLIC char* HTLocalName(const char* name) {
    ...
    if (0 == strcmp(access, "file")) { /* local file */
        free(access);
        if ((0 == strcasecomp(host, HTHostName())) || (0 == strcasecomp(host, "localhost")) ||
            !*host) {
            free(host);
            if (TRACE)
            ...
            char* result = (char*)malloc(strlen("/Net/") + strlen(host) + strlen(path) + 1);
            if (result == NULL)
...

{
    ...
    *pencoding = NULL;
    ...
    if (!*pencoding)
        *pencoding = suff->encoding ? suff->encoding : HTAtom_for("binary");
...
PUBLIC BOOL HTEditable(const char* filename) {
    ...
    if (TRACE) {
        int i;
        fprintf(stderr, "File mode is 0%o, uid=%d, gid=%d. My uid=%d, %d groups (",
                ...
                */
                    ...
                    do_readme(target, localname);
                {
                    ...
                    while ((dirbuf = readdir(dp)) != 0) {
                        ...
                        dirname = (HTBTElement*)malloc(strlen(dirbuf->d_name) + 2);
                        if (dirname == NULL)
                            outofmem(__FILE__, "DirRead");
                     ...
                     */
                    {
                        ...
                        while (next_element != NULL) {
                            ...
                            if (strcmp((char*)(HTBTree_object(next_element)), "D..")) {
                                if (state != *(char*)(HTBTree_object(next_element))) {
                                    if (state == 'D')
                                        END(HTML_DIR);
                                    state =
                                        *(char*)(HTBTree_object(next_element)) == 'D' ? 'D' : 'F';
                                    START(HTML_H2);
                            ...
                            HTDirEntry(target, tail, (char*)HTBTree_object(next_element) + 1);
