...
#endif
{
    ...
    for (r = rules; r; r = r->next) {
        ...
        case HT_Protect: {
            ...
            if (TRACE)
                fprintf(stderr, "HTRule: `%s' matched %s %s: `%s'\n", current,
                        (r->op == HT_Protect ? "Protect" : "DefProt"), "rule, setup",
                        (r->equiv ? r->equiv : (r->op == HT_Protect ? "DEFAULT" : "NULL!!")));
...
PUBLIC int HTSetConfiguration ARGS1(const char*, config) {
    ...
    if (0 == strcasecomp(word1, "defprot") || 0 == strcasecomp(word1, "protect"))
    ...
    if (0 == strcasecomp(word1, "suffix")) {
        ...
        if (pointer)
            ...
            status = 0;
        HTSetSuffix(word2, word3, encoding ? encoding : "binary", status >= 1 ? quality : 1.0);
        ...
        if (pointer)
            ...
            status = 0;
        HTSetPresentation(word2, word3, status >= 1 ? quality : 1.0, status >= 2 ? secs : 0.0,
                          status >= 3 ? secs_per_byte : 0.0);

    } else if (0 == strncasecomp(word1, "htbin", 5) || 0 == strncasecomp(word1, "bindir", 6)) {
...
int HTLoadRules ARGS1(const char*, filename) {
    ...
    if (!fp) {
        if (TRACE)
            printf("HTRules: Can't open rules file %s\n", filename);
