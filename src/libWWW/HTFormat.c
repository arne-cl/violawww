...
PUBLIC BOOL HTPushInputBuffer NOARGS {
    ...
    if (doInitialize) {
        int i;
        for (i = 0; i < BUFF_INFO_SIZE; i++)
            input_buffer_stack[i] = NULL;
        doInitialize = 0;
    }
    ...
    if (!input_buffer_stack[input_buffer_stacki]) {
        input_buffer_stack[input_buffer_stacki] = (char*)malloc(sizeof(char) * INPUT_BUFFER_SIZE);
    }
...
*/
PUBLIC HTStream* HTStreamStack ARGS4(HTFormat, rep_in, HTFormat, rep_out, HTStream*, sink,
                                     HTParentAnchor*, anchor) {
    HTAtom* wildcard = HTAtom_for("*");
    HTFormat source = WWW_SOURCE;
    if (TRACE)
        fprintf(stderr, "HTFormat: Constructing stream stack for %s to %s\n", HTAtom_name(rep_in),
                HTAtom_name(rep_out));
...

    {
        ...
        int i;
        HTPresentation *pres, *match, *wildcard_match = 0, *source_match = 0,
                                      *source_wildcard_match = 0;
        for (i = 0; i < n; i++) {
...
*/
PUBLIC float HTStackValue ARGS4(HTFormat, rep_in, HTFormat, rep_out, float, initial_value, long int,
                                length) {
    HTAtom* wildcard = HTAtom_for("*");
...
**
*/
PUBLIC int HTParseSocket ARGS5(HTFormat, rep_in, HTFormat, format_out, HTParentAnchor*, anchor, int,
                               file_number, HTStream*, sink) {
    HTStream* stream;
    HTStreamClass targetClass;
    ...
    if (!stream) {
        char buffer[1024]; /* @@@@@@@@ */
        sprintf(buffer, "Sorry, can't convert from %s to %s.", HTAtom_name(rep_in),
    ...
    */
    ...
    if (rep_in == WWW_BINARY || HTOutputSource || strstr(HTAtom_name(rep_in), "image/") ||
...
**
*/
PUBLIC int HTParseFile ARGS5(HTFormat, rep_in, HTFormat, format_out, HTParentAnchor*, anchor, FILE*,
                             fp, HTStream*, sink) {
    HTStream* stream;
    HTStreamClass targetClass;
...
PRIVATE HTStreamClass NetToTextClass = {
    "NetToText",        NetToText_free,          NetToText_end,
    NetToText_abort,    NetToText_put_character, NetToText_put_string,
    NetToText_progress, NetToText_put_block,
};
