...
#define START(e) (*me->target->isa->start_element)(me->target, e, 0, 0, &HTML_dtd.tags[e] /*PYW*/)
...
PRIVATE BOOL write_cache ARGS1(HTStream*, me) {
    ...
    sprintf(cache_file_name, "%sWSRC-%s:%s:%.100s.txt", CACHE_FILE_PREFIX,
...

{
    ...
    if (source_file) {
        ...
        if (me->par_value[PAR_IP_NAME] && me->par_value[PAR_DATABASE_NAME]) {
            ...
            char* www_database;
            www_database = HTEscape(me->par_value[PAR_DATABASE_NAME], URL_XALPHAS);
            sprintf(WSRC_address, "wais://%s:%s/%s", me->par_value[PAR_IP_NAME],
            ...
            sprintf(WSRC_address, "http://info.cern.ch:8001/%s:%s/%s", me->par_value[PAR_IP_NAME],
                    me->par_value[PAR_TCP_PORT] ? me->par_value[PAR_TCP_PORT] : "210",
                    www_database);
            HTStartAnchor(me->target, NULL, WSRC_address);
...
PRIVATE void WSRCParser_abort ARGS2(HTStream*, me, HTError, e) { WSRCParser_free(me); }
...
HTStreamClass WSRCParserClass = {
    "WSRCParser",        WSRCParser_free,          NULL, /* end */
    WSRCParser_abort,    WSRCParser_put_character, WSRCParser_put_string,
    WSRCParser_progress, WSRCParser_write
...
**		--------------------------------------
*/
PUBLIC HTStream* HTWSRCConvert ARGS3(HTPresentation*, pres, HTParentAnchor*, anchor, HTStream*,
                                     sink) {
