...
PUBLIC void FOSIUseCharacterSet ARGS1(FOSICharacterSet, i) {
    p_entity_values = (i == FOSI_NEXT_CHARS) ? NeXTCharacters : ISO_Latin1;
}
...
*/
PUBLIC HTStructured* FOSI_new ARGS2(HTParentAnchor*, anchor, HTStream*, stream) {
    ...
    this->target = stream;
    if (stream)
        this->targetClass = *stream->isa; /* Copy pointers */
