...
#else
PRIVATE void handle_attribute_name(context, s) HTStream* context;
char* s;
#endif
{
...
#else
PRIVATE void handle_attribute_value(context, s) HTStream* context;
char* s;
#endif
{
...
#else
PRIVATE void handle_entity(context, term) HTStream* context;
char term;
#endif
{
...
#else
PRIVATE void end_element(context, old_tag) HTTag* old_tag;
HTStream* context;
#endif
{
    if (TRACE)
    ...
    while (context->element_stack) { /* Loop is error path only */
        ...
        if (old_tag != t) {                     /* Mismatch: syntax error */
            if (context->element_stack->next) { /* This is not the last level */
                if (TRACE)
                    fprintf(stderr, "SGML: Found </%s> when expecting </%s>. </%s> assumed.\n",
                            old_tag->name, t->name, t->name);
            } else { /* last level */
                if (TRACE)
                    fprintf(stderr, "SGML: Found </%s> when expecting </%s>. </%s> Ignored.\n",
...
#else
PRIVATE void close_element(context, old_tag) HTStream* context;
HTTag* old_tag;
#endif
{
...
#else
PRIVATE void start_element(context) HTStream* context;
#endif
{
...
PUBLIC void SGML_end ARGS1(HTStream*, context) { (*context->actions->end)(context->target); }
...
#ifdef CALLERDATA
PUBLIC void* SGML_callerData ARGS1(HTStream*, context) { return context->callerData; }
...
PRIVATE void closeImpliedTags ARGS2(HTStream*, context, HTTag*, tag) {
...
#endif
    for (low = 0, high = context->element_stack->tag->valid_sub_elementsCount; high > low;
         diff < 0 ? (low = i + 1) : (high = i)) {
        i = (low + (high - low) / 2);
        diff = *(subep + i) - tagIdx;
        if (diff == 0) {
    ...
    if (implyETagP) {
        ...
        while (context->element_stack) {
            ...
            if (bottom == NULL) {
#ifdef IMPLY_VERBOSE
                fprintf(stderr, "::: empty stack. BREAK\n");
#endif
                break;
            }
#ifdef IMPLY_VERBOSE
            fprintf(stderr, "::: bottom-tag(n)=%s  tag=%s\n", bottom->tag->name, tagName);
#endif
            subep = (int*)bottom->tag->valid_sub_elements;
            if (subep) {

                for (low = 0, high = bottom->tag->valid_sub_elementsCount; high > low;
                     diff < 0 ? (low = i + 1) : (high = i)) {
                    i = (low + (high - low) / 2);
                    diff = *(subep + i) - tagIdx;
                    if (diff == 0) {
...

{
    ...
    */
    ...
    case S_text: /* !     PYW:  seems either this or change <A>'s
        ...
        if (c == '&' && (context->element_stack && context->element_stack->tag &&
            ...
            */
            ...
            string->size = 0;
            context->state = (context->element_stack && context->element_stack->tag &&
    ...
    case S_litteral:
        ...
        if (TOUPPER(c) !=
            ((string->size == 1) ? '/' : context->element_stack->tag->name[string->size - 2])) {
            int i;
    ...
    case S_tag: /* new tag */
        if (isalnum(c))
            ...
            if (t /* context->current_tag*/ && /*PYW*/
                context->element_stack && context->element_stack->tag) {

                /*		char *newtag = dtd->tags[t].name;*/ /* same t as old lib??*/
                char* newtag = t->name;
#ifdef IMPLY_VERBOSE
                fprintf(stderr, "Stag implications: Oldtag=%s, Newtag=%s\n",
                        context->element_stack->tag->name, newtag);
#endif
                ...
                if (context->current_tag) {
                    ...
                    if (context->element_stack && context->element_stack->tag) {
                        fprintf(stderr, "Etag implications: Oldtag=%s, Endtag=%s\n",
                                context->element_stack->tag->name, closeTag);
                    }
#endif
