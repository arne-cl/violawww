...
#define START(e) (*target->isa->start_element)(target, e, 0, 0, &HTML_dtd.tags[e] /*PYW*/)
...
char* WWW_from_archie ARGS1(char*, file) {
    ...
    if (colon) {
        for (; colon[0]; colon[0] = colon[1], colon++)
            ; /* move down */
    }
    return result;
} /* WWW_from_archie */
...
PRIVATE void output_text_record ARGS4(HTStream*, target, WAISDocumentText*, record, boolean,
    ...
    if (binary) {
        (*target->isa->put_block)(target, record->DocumentText->bytes, record->DocumentText->size);
        return;
    }
 ...
 */
void display_search_response ARGS4(HTStructured*, target, SearchResponseAPDU*, response, char*,
                ...
                if (archie) {
                    ...
                    if (docname) {
                        ...
                        HTStartAnchor(target, NULL,
                                      ((head->Types) && (!strcmp(head->Types[0], "URL")))
                                          ? headline
                                          : line); /* NT, Sep 93 */
                        PUTS(headline);
                        END(HTML_A);
...

{
    ...
    */
    ...
    if (server_name[0] == 0)
        ...
        if ((connection = connect_to_server(server_name, atoi(service))) == NULL) {
            if (TRACE)
                fprintf(stderr, "%sCan't open connection to %s via service %s.\n", error_header,
                        server_name, service);
            free(names);
    ...
    */
    ...
    if (key && !*key) { /* I N D E X */
...
#ifdef CACHE_FILE_PREFIX
        sprintf(filename, "%sWSRC-%s:%s:%.100s.txt", CACHE_FILE_PREFIX, server_name, service,
                www_database);

        fp = fopen(filename, "r"); /* Have we found this already? */
        if (TRACE)
            fprintf(stderr, "HTWAIS: Description of server %s %s.\n", filename,
        ...
        if (NULL == generate_search_apdu(request_message + HEADER_LENGTH, &request_buffer_length,
                                         keywords, wais_database, NULL, MAXDOCS))
            panic("request too large");

        if (!interpret_message(request_message, MAX_MESSAGE_LEN - request_buffer_length,
        ...
        any* docid = &doc_chunk;
        if (TRACE)
            printf("HTWAIS: Retrieve document id `%s' type `%s' length %ld\n", docname, doctype,
                   document_length);

        format_in = !strcmp(doctype, "WSRC")   ? HTAtom_for("application/x-wais-source")
        ...
        if (!target)
            return HTLoadError(sink, 500, "Can't convert format of WAIS document");
        ...
        for (count = 0; count * CHARS_PER_PAGE < document_length; count++) {
            ...
            if (0 == generate_retrieval_apdu(
                         request_message + HEADER_LENGTH, &request_buffer_length, docid, CT_byte,
                         count * CHARS_PER_PAGE, MIN((count + 1) * CHARS_PER_PAGE, document_length),
                         type, wais_database))
                panic("request too long");

            /*	Actually do the transaction given by request_message */
            if (0 == interpret_message(request_message, MAX_MESSAGE_LEN - request_buffer_length,
                                       response_message, MAX_MESSAGE_LEN, connection,
                                       false /* true verbose */
                                       ))
                panic("Returned message too large");
            ...
            if (NULL ==
                ...
                output_text_record(
                    target,
                    ((WAISSearchResponse*)retrieval_response->DatabaseDiagnosticRecords)->Text[0],
                    false, binary);
