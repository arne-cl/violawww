...
PRIVATE void syntax_error ARGS3(FILE*, fp, char*, msg, LexItem, lex_item) {
    ...
    if (TRACE)
        fprintf(stderr, "%s %d before: '%s'\nHTGroup.c: %s (got %s)\n",
                "HTGroup.c: Syntax error in rule file at line", lex_line, buffer, msg,
...
PRIVATE AddressDefList* parse_address_part ARGS1(FILE*, fp) {
    ...
    if (lex_item == LEX_ALPH_STR || lex_item == LEX_TMPL_STR)
        only_one = YES;
             ...
             ((lex_item = lex(fp)) != LEX_ALPH_STR && lex_item != LEX_TMPL_STR)) {
...
PRIVATE BOOL ip_in_def_list ARGS3(AddressDefList*, address_def_list, char*, ip_number, char*,
                                  ip_name) {
    ...
    if (!group_cache_list)
        ...
        while (NULL != (group_cache = (GroupCache*)HTList_nextObject(cur))) {
            if (!strcmp(filename, group_cache->group_filename)) {
                if (TRACE)
                    fprintf(stderr, "%s '%s' %s\n", "HTAA_readGroupFile: group file", filename,
...
*/
PUBLIC HTAAFailReasonType HTAA_userAndInetInGroup ARGS4(GroupDef*, group, char*, username, char*,
                                                        ip_number, char*, ip_name) {
                ...
                if (!item->user_def_list) /* Any user allowed */
                    ...
                    while (NULL != (ref = (Ref*)HTList_nextObject(cur2))) {
                        ...
                        if (ref->translation) { /* Group, check recursively */
                            reason = HTAA_userAndInetInGroup(ref->translation, username, ip_number,
                                                             ip_name);
                            if (reason == HTAA_OK)
                                return HTAA_OK;
