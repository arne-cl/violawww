...
static MenuItem clonePageMenuItems[] = {
    {"Close", &xmPushButtonWidgetClass, 'C', "Ctrl<Key>C", "Ctrl-C", closePageCB, NULL,
     "Close this window", TRUE, (MenuItem*)NULL},
    NULL,
};
...
void clonePage(DocViewInfo* parentDocViewInfo) {
    ...
    shell = XtVaAppCreateShell(localDocViewInfo->shellName
                               ...
                               "Viola", applicationShellWidgetClass,
                               XtDisplay(parentDocViewInfo->shell), XmNborderWidth, 2,
    ...
    closeB = XtVaCreateManagedWidget(
    ...
    titleFrame = XtVaCreateManagedWidget(
        ...
        closeB, XmNleftOffset, 4, XmNrightAttachment, XmATTACH_FORM, XmNmarginWidth, 2,
    ...
    scrollBar = XtVaCreateManagedWidget("scrollBar", xmScrollBarWidgetClass, frame,
...
void showPageClone(char* arg[], int argc, void* clientData) {
    ...
    XtAddCallback(dvi->scrollBar, XmNdragCallback, scrollBarDrag, (XtPointer)dvi->violaDocViewObj);
    XtAddCallback(dvi->scrollBar, XmNvalueChangedCallback, scrollBarValueChanged,
                  (XtPointer)dvi->violaDocViewObj);
    XtAddEventHandler(dvi->canvas, StructureNotifyMask, FALSE, resizeViola,
...
void cloneApp(Widget button, XtPointer clientData, XtPointer callData) {
    ...
    shell = XtVaAppCreateShell(nameBuf, nameBuf, applicationShellWidgetClass, XtDisplay(button),
                               XmNborderWidth, 2, XmNiconPixmap, icon, NULL);
    dvi = makeBrowserInterface(shell, nameBuf, dvi, 0, NULL);

    cd = (ClientData*)malloc(sizeof(ClientData));
    cd->data = (void*)dvi->violaDocViewName;
    cd->shellInfo = (void*)dvi;
    XtAddEventHandler(dvi->canvas, StructureNotifyMask, FALSE, appCloneMapped, (XtPointer)cd);
...
void quitCallback(Widget widget, XtPointer client, XtPointer call) {
    ...
    if (!dvi->quitDlog) {
        if (dvi->shell == topWidget) {
            ...
            n = 0;
            XtSetArg(args[n], XmNmessageString, xms);
            n++;
            ...
            dlog = XmCreateQuestionDialog(topWidget, "Confirm Quit", args, n);
            XmStringFree(xms);
            ...
            xms = XmStringCreateLtoR(
                "Only Close this Particular Viewer or Exit the Entire Program?", "largeFont");
            ...
            XtSetArg(args[n], XmNmessageString, xms);
            n++;
            XtSetArg(args[n], XmNokLabelString, okLabel);
            n++;
            XtSetArg(args[n], XmNcancelLabelString, cancelLabel);
            n++;
            XtSetArg(args[n], XmNhelpLabelString, helpLabel);
            n++;
            ...
            n++;
            dlog = XmCreateQuestionDialog(dvi->shell, "Confirm Quit", args, n);
            XmStringFree(xms);
            ...
            button = XmMessageBoxGetChild(dlog, XmDIALOG_OK_BUTTON);
            XtAddCallback(button, XmNactivateCallback, closeThisShell, dvi);
...
void genericCallback(Widget thing, XtPointer clientData, XtPointer callData) {
    ...
    sprintf(msg, "Selected the \"%s\" item.", ((char*)(((ClientData*)clientData)->data)));
...
void openDocument(Widget widget, XtPointer clientD, XtPointer callData) {
    ...
    if (!dlog) {
        ...
        XtSetArg(args[n], XmNshadowType, XmSHADOW_ETCHED_OUT);
        n++;
        dlog = XmCreateFileSelectionDialog(dvi->shell, "Open Document", args, n);
        XmStringFree(xms);
        ...
        XtVaSetValues(dlog, XmNokCallback, (XtCallbackList)okCallback, XmNcancelCallback,
                      (XtCallbackList)cancelCallback, XmNhelpCallback, (XtCallbackList)helpCallback,
                      NULL);
        XtManageChild(dlog);
    }
...
void scrollBarConfig(Widget sbar, int thumbPercent, int positionPercent)
{
    ...
    XtVaSetValues(sbar, XmNsliderSize, (thumbPercent * maximum / SBAR_MAGNITUDE), XmNvalue,
...
void scrollBarValueChanged(Widget sbar, XtPointer clientData, XtPointer callData) {
    ...
    if (calledDrag)
        ...
        if (!sbar) {
            fprintf(stderr, "internal error occured in scrollBarValueChanged(sbar==NULL,,,)\n");
...
void sliderConfig(char* arg[], int argc, void* clientData) {
    ...
    XtVaSetValues(dvi->scrollBar, XmNsliderSize, sliderSize, XmNpageIncrement, sliderSize, XmNvalue,
                  sliderPos, NULL);
}
