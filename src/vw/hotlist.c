...
void hotlistAdd(DocViewInfo* dvi, char* newItem) {
    ...
    if (dvi->hotlistListWidget) {
        ...
        XmListDeselectAllItems(dvi->hotlistListWidget);
        XmListSelectPos(dvi->hotlistListWidget, dvi->currentHotlistItem + 1, FALSE);
    }
}
...
int numItems;
{
    ...
    if (dvi->hotlistListWidget) {
        ...
        XtVaSetValues(dvi->hotlistListWidget, XmNitemCount, numItems, XmNitems, listItems, NULL);
        free(listItems);
    }
#endif
}
...
Widget createHotlistDialog(DocViewInfo* dvi) {
    ...
    addButton = XtVaCreateManagedWidget(
        "Add Current", xmPushButtonWidgetClass, actionForm, XmNtopAttachment, XmATTACH_FORM,
        XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment,
        XmATTACH_POSITION, XmNrightPosition, 1, NULL);
    XtAddCallback(addButton, XmNactivateCallback, addHotlistItem, (XtPointer)dvi);

    deleteButton = XtVaCreateManagedWidget(
        "Delete", xmPushButtonWidgetClass, actionForm, XmNtopAttachment, XmATTACH_FORM,
        XmNbottomAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_POSITION,
        XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, addButton, XmNrightPosition, 2, NULL);
    XtAddCallback(deleteButton, XmNactivateCallback, deleteHotlistItem, (XtPointer)dvi);

    gotoButton = XtVaCreateManagedWidget(
        "Goto", xmPushButtonWidgetClass, actionForm, XmNtopAttachment, XmATTACH_FORM,
        XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_WIDGET, XmNrightAttachment,
        XmATTACH_POSITION, XmNleftWidget, deleteButton, XmNrightPosition, 3, NULL);
    XtAddCallback(gotoButton, XmNactivateCallback, gotoHotlistItem, (XtPointer)dvi);

    editButton = XtVaCreateManagedWidget(
        "Edit Label", xmPushButtonWidgetClass, actionForm, XmNtopAttachment, XmATTACH_FORM,
        XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_WIDGET, XmNrightAttachment,
        XmATTACH_FORM, XmNrightPosition, 4, XmNleftWidget, gotoButton, NULL);
    XtAddCallback(editButton, XmNactivateCallback, editHotlistItem, (XtPointer)dvi);
    ...
    XtAddCallback(doneButton, XmNactivateCallback, hideHotlist, (XtPointer)dvi);
    ...
    n = 0;
    {
        ...
        n++;
        XtSetArg(args[n], XmNitemCount, i);
        n++;
    }
    XtSetArg(args[n], XmNtopAttachment, XmATTACH_WIDGET);
    n++;
    XtSetArg(args[n], XmNtopWidget, actionForm);
    n++;
...
XtPointer clientData, callData;
{
    ...
    if (dvi->currentHotlistItem == cbs->item_position - 1) {
        if (dvi->violaDocViewObj) {
            ...
            sendMessage1N1int(dvi->violaDocViewObj, "hotlistList_goto", dvi->currentHotlistItem);
...
void addToHotlist(DocViewInfo* dvi) {
    if (dvi->violaDocViewObj) {
        setBusyCursor_history(dvi);
        clearPacket(&pk);
        sendMessage1N1int_result(dvi->violaDocViewObj, "hotlistList_add", dvi->currentHotlistItem,
                                 &pk);
...
XtPointer clientData, callData;
{
    ...
    if (dvi->violaDocViewObj) {
        if (dvi->currentHotlistItem >= 0) {
            ...
            sendMessage1N1int(dvi->violaDocViewObj, "hotlistList_delete", dvi->currentHotlistItem);
