...
XtPointer clientData, callData;
{
     ...
     */
    ViolaRegisterMessageHandler("showSourceString", showSourceStringMH, (void*)dvi);
    ViolaRegisterMessageHandler("showSourceFile", showSourceFileMH, (void*)dvi);
...
void* clientData;
{
    ...
    ViolaDeleteMessageHandler("showSourceString", showSourceStringMH, clientData);
    ViolaDeleteMessageHandler("showSourceFile", showSourceFileMH, clientData);
...
char* readFile(char* fileName, DocViewInfo* dvi) {
    ...
    while (!feof(fp) && !ferror(fp) && bytesRead < bytesInFile) {
        bytes = fread((char*)(fileData + bytesRead), 1, (bytesInFile - bytesRead), fp);
        bytesRead += bytes;
    }
    fclose(fp);
...
int editable;
{
    ...
    shell = XtVaAppCreateShell(shellName, "Viola", applicationShellWidgetClass,
    ...
    XtAddEventHandler(title, ButtonReleaseMask, FALSE, editorTitleButtonEH,
    ...
    */
    ...
    reloadB = XtVaCreateManagedWidget(
        "Upload", xmPushButtonWidgetClass, buttonBox, XmNtopAttachment, XmATTACH_FORM,
        XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment,
        XmATTACH_POSITION, XmNrightPosition, 1, XmNsensitive, FALSE,
        /*XmNsensitive, (editable? TRUE: FALSE),*/
        NULL);
    XtAddCallback(reloadB, XmNactivateCallback, reloadSource, (XtPointer)cd);

    saveB = XtVaCreateManagedWidget("Save", xmPushButtonWidgetClass, buttonBox, XmNtopAttachment,
                                    XmATTACH_FORM, XmNbottomAttachment, XmATTACH_FORM,
                                    XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, reloadB,
                                    XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition, 2,
                                    XmNsensitive, FALSE, NULL);
    XtAddCallback(saveB, XmNactivateCallback, saveSource, (XtPointer)cd);
    ...
    saveAsB = XtVaCreateManagedWidget(
        ...
        saveB, XmNrightAttachment, XmATTACH_POSITION, XmNrightPosition, 3, XmNsensitive,
        (editable ? TRUE : FALSE), NULL);
    XtAddCallback(saveAsB, XmNactivateCallback, saveAsSource, (XtPointer)localDocViewInfo);
    ...
    XtAddCallback(closeB, XmNactivateCallback, closeSourceEditor, (XtPointer)cd);

    helpLabel = XtVaCreateManagedWidget(
        "helpLabel", xmLabelWidgetClass, form, XmNbottomAttachment, XmATTACH_WIDGET,
        XmNbottomWidget, buttonBox, XmNrightAttachment, XmATTACH_FORM, XmNleftAttachment,
        XmATTACH_FORM, XmNrecomputeSize, FALSE, XmNborderWidth, 1, NULL);
    setHelp(title, helpLabel,
            "Click right mouse button to copy the document URL to the clipboard.");
    setHelp(reloadB, helpLabel,
            "Reloads this source into the WWW Document Viewer.  Changes you have made will be "
            "reflected.");
    ...
    frame = XtVaCreateManagedWidget(
        "frame", xmFormWidgetClass, form, XmNtopAttachment, XmATTACH_WIDGET, XmNtopWidget,
        titleFrame, XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget, helpLabel,
        XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNshadowThickness, 4,
        XmNshadowType, XmSHADOW_ETCHED_IN, XmNhorizontalSpacing, 4, XmNverticalSpacing, 4, NULL);
    ...
    n++;
    XtSetArg(args[n], XmNtopWidget, titleFrame);
    n++;
    ...
    n++;
    XtSetArg(args[n], XmNeditable, (editable ? TRUE : FALSE));
    n++;
    XtSetArg(args[n], XmNeditMode, XmMULTI_LINE_EDIT);
    n++;
    XtSetArg(args[n], XmNvalue, data);
    n++;
    textEditor = XmCreateScrolledText(frame, "textEditor", args, n);
    XtManageChild(textEditor);
    setHelp(textEditor, helpLabel,
            "This is the source text from which the current document was generated.");
    ...
    if (editable)
        XtAddCallback(textEditor, XmNvalueChangedCallback, editorValueChangedCB,
                      (XtPointer)localDocViewInfo);
    ...
    if (dvi->editorDataChanged) {
        ...
        if (!strcmp(answer, "Save Changes...")) {
            answer = questionDialog(dvi,
                                    ...
                                    "the document or to a new file?",
                                    "Save As a File...", "Save to Original Source",
                                    "Save As a File...", "Cancel");
            if (!strcmp(answer, "Save to Original Source")) {
                ...
                if (vwSaveAs("Saving document source to a file...\n\nEnter the full path name:",
                    ...
                    XtAddCallback(dvi->textEditor, XmNvalueChangedCallback, editorValueChangedCB,
                                  (XtPointer)dvi);
                }
            } else {
                return;
...
DocViewInfo* dvi;
{
    ...
    if (!fp) {
        ...
        while (bytesLeft > 0 && !ferror(fp))
            bytesLeft -= fwrite((textData + numBytes - bytesLeft), sizeof(char), bytesLeft, fp);
        ...
        if (length > 6) {
            ...
            if (!strcmp(answer, "Yes")) {
                ...
                if (writeStatus) {
                    ...
                    XtAddCallback(dvi->textEditor, XmNvalueChangedCallback, editorValueChangedCB,
                                  (XtPointer)dvi);
                    XtVaSetValues(dvi->editorSaveButton, XmNsensitive, FALSE, NULL);
                }
            }
        }
    } else {
        modalErrorDialog(dvi, "Sorry, it is not yet possible to upload document\nsource changes to "
                              "a remote server.  (So go away!)");
        return;
