...
static MenuItem catalogFileMenuItems[] = {
    {"Open", &xmPushButtonWidgetClass, 'O', "Ctrl<Key>O", "Ctrl-O", NULL, (XtPointer)NULL,
     "Open the selected folder or link.", TRUE, (MenuItem*)NULL},
    {"Close", &xmPushButtonWidgetClass, 'Q', "Ctrl<Key>Q", "Ctrl-Q", closeCatalogCB,
     (XtPointer)NULL, "Close this catalog window.", TRUE, (MenuItem*)NULL},
    NULL,
};

static MenuItem catalogEditMenuItems[] = {
    {"Copy", &xmPushButtonWidgetClass, 'C', "Ctrl<Key>C", "Ctrl-C", genericCallback, "Copy",
     "Copy the selection to the clipboard.", TRUE, (MenuItem*)NULL},
    {"Paste", &xmPushButtonWidgetClass, 'P', "Ctrl<Key>V", "Ctrl-V", genericCallback, "Paste",
     "Paste the clipboard to the current selection.", TRUE, (MenuItem*)NULL},
    NULL,
};

static MenuItem catalogCatalogMenuItems[] = {
    {"Clean Up Window", &xmPushButtonWidgetClass, 'W', "Ctrl<Key>W", "Ctrl-W", genericCallback, "",
     "Neatly organize the icons in the current folder.", TRUE, (MenuItem*)NULL},
    {"Alphabetize Window", &xmPushButtonWidgetClass, 'A', "Ctrl<Key>A", "Ctrl-A", genericCallback,
     "", "Organize icons alphabetically.", TRUE, (MenuItem*)NULL},
    {"Info", &xmPushButtonWidgetClass, 'I', "Ctrl<Key>I", "Ctrl-I", genericCallback, "",
     "View and edit the comments about the selected link or folder.", TRUE, (MenuItem*)NULL},
    {"Modify", &xmPushButtonWidgetClass, 'M', "Ctrl<Key>M", "Ctrl-M", genericCallback, "",
     "Modify the selected link or folder.", TRUE, (MenuItem*)NULL},
    NULL,
};

static MenuItem catalogHelpMenuItems[] = {
    {"Help on a Helpful Topic", &xmPushButtonWidgetClass, 'H', "Ctrl<Key>H", "Ctrl-H",
     genericCallback, "Help on a Helpful Topic", "Place holder for the help menu items.", TRUE,
     (MenuItem*)NULL},
    NULL,
};
...
void drawFolder(Widget canvas, Folder* folder) {
    ...
    XmStringDraw(XtDisplay(canvas), XtWindow(canvas), bogusFontList, folder->nameXMS, folder->gc,
                 ...
                 (XRectangle*)NULL);
    XmStringDraw(XtDisplay(canvas), XtWindow(canvas), bogusFontList, folder->nItemsXMS, folder->gc,
                 folder->numx, folder->numy, 1000, XmALIGNMENT_BEGINNING, XmSTRING_DIRECTION_L_TO_R,
                 (XRectangle*)NULL);
}
...
void forceDeleteLink(Link* link, Catalog* catalog) {
    ...
    if (link->name)
        free(link->name);
    if (link->nameXMS)
        XmStringFree(link->nameXMS);
    if (link->commentURL)
        free(link->commentURL);
    if (link->url)
        free(link->url);
    if (link->comment)
        free(link->comment);
...
void forceDeleteFolder(Folder* folder, Catalog* catalog) {
    if (folder) {
        ...
        if (folder->items)
            free(folder->items);
        if (folder->iconFile)
            free(folder->iconFile);
        if (folder->commentURL)
            free(folder->commentURL);
        if (folder->name)
            free(folder->name);
        if (folder->nameXMS)
            XmStringFree(folder->nameXMS);
        if (folder->nItemsXMS)
            XmStringFree(folder->nItemsXMS);
        free(folder);
    }
}
...
void deleteFolder(Folder* folder, Catalog* catalog) {
    ...
    if (folder->nItems) {
        ...
        answer = questionDialog(catalog->dvi, question, "Cancel", "Delete Folder", "Cancel", NULL);
        if (answer[0] == 'C')
            return;
    }
...
void showCatalog(char* catalogFile, DocViewInfo* parentDVI) {
    ...
    Widget shell, mainWindow, form, helpLabel, titleFrame, folderNameLabel, folderNameText,
        catalogTitleLabel, catalogTitleText, catalogFileLabel, catalogFileText, frame, menubar,
        canvasFrame, canvas;
    ...
    shell = XtVaAppCreateShell("Personal URL Catalog", "Personal URL Catalog",
                               ...
                               XmNborderWidth, 2, XmNiconPixmap, icon, XmNshadowType,
                               XmSHADOW_ETCHED_OUT, XmNshadowThickness, 2, NULL);

    initCatalog(shell);

    mainWindow = XmCreateMainWindow(shell, "mainWindow", NULL, 0);

    form = XtVaCreateManagedWidget("form", xmFormWidgetClass, mainWindow, XmNhorizontalSpacing,
                                   FORM_SPACING, XmNverticalSpacing, FORM_SPACING, NULL);

    xms = XmStringCreateLtoR("Personal URL Catalog", "helpFont");
    helpLabel = XtVaCreateManagedWidget("helpLabel", xmLabelWidgetClass, form, XmNleftAttachment,
                                        XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
                                        XmNbottomAttachment, XmATTACH_FORM, XmNborderWidth, 1,
                                        XmNrecomputeSize, FALSE, XmNlabelString, xms, NULL);
    XmStringFree(xms);
    setHelp(helpLabel, helpLabel, "This is the message area.");

    titleFrame = XtVaCreateManagedWidget(
        "titleFrame", xmFormWidgetClass, form, XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment,
        XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNshadowThickness, 1, XmNshadowType,
    ...
    folderNameLabel = XtVaCreateManagedWidget("folderNameLabel", xmLabelWidgetClass, titleFrame,
                                              XmNtopAttachment, XmATTACH_FORM, XmNleftAttachment,
                                              XmATTACH_FORM, XmNalignment, XmALIGNMENT_BEGINNING,
                                              XmNrecomputeSize, FALSE, XmNlabelString, xms, NULL);
    XmStringFree(xms);

    xms = XmStringCreateLtoR("Foobar Folder", "plainFont");
    folderNameText = XtVaCreateManagedWidget(
        "folderNameText", xmLabelWidgetClass, titleFrame, XmNtopAttachment, XmATTACH_FORM,
        XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET, XmNbottomWidget, folderNameLabel,
        XmNbottomOffset, 0, XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget, folderNameLabel,
        XmNrightAttachment, XmATTACH_FORM, XmNalignment, XmALIGNMENT_BEGINNING, XmNrecomputeSize,
        FALSE, XmNlabelString, xms, NULL);
    XmStringFree(xms);
    ...
    catalogTitleLabel = XtVaCreateManagedWidget(
        "catalogTitleLabel", xmLabelWidgetClass, titleFrame, XmNtopAttachment, XmATTACH_WIDGET,
        XmNtopWidget, folderNameLabel, XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment,
        XmATTACH_OPPOSITE_WIDGET, XmNrightWidget, folderNameLabel, XmNrightOffset, 0, XmNalignment,
        XmALIGNMENT_BEGINNING, XmNrecomputeSize, FALSE, XmNlabelString, xms, NULL);
    XmStringFree(xms);

    xms = XmStringCreateLtoR("Foobar catalog of things and stuff.", "plainFont");
    catalogTitleText = XtVaCreateManagedWidget(
        "catalogTitleText", xmLabelWidgetClass, titleFrame, XmNtopAttachment, XmATTACH_WIDGET,
        XmNtopWidget, folderNameLabel, XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET,
        XmNbottomWidget, catalogTitleLabel, XmNbottomOffset, 0, XmNleftAttachment, XmATTACH_WIDGET,
        XmNleftWidget, catalogTitleLabel, XmNrightAttachment, XmATTACH_FORM, XmNalignment,
        XmALIGNMENT_BEGINNING, XmNrecomputeSize, FALSE, XmNlabelString, xms, NULL);
    XmStringFree(xms);
    ...
    catalogFileLabel = XtVaCreateManagedWidget(
        "catalogFileLabel", xmLabelWidgetClass, titleFrame, XmNtopAttachment, XmATTACH_FORM,
        XmNtopWidget, catalogTitleLabel, XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment,
        XmATTACH_FORM, XmNrightAttachment, XmATTACH_OPPOSITE_WIDGET, XmNrightWidget,
        folderNameLabel, XmNrightOffset, 0, XmNrecomputeSize, FALSE, XmNalignment,
        XmALIGNMENT_BEGINNING, XmNlabelString, xms, NULL);
    XmStringFree(xms);

    xms = XmStringCreateLtoR(catalogFile, "plainFont");
    catalogFileText = XtVaCreateManagedWidget(
        "catalogFileText", xmLabelWidgetClass, titleFrame, XmNtopAttachment, XmATTACH_WIDGET,
        XmNtopWidget, catalogTitleLabel, XmNbottomAttachment, XmATTACH_OPPOSITE_WIDGET,
        XmNbottomWidget, catalogFileLabel, XmNbottomOffset, 0, XmNleftAttachment, XmATTACH_WIDGET,
        XmNleftWidget, catalogFileLabel, XmNrightAttachment, XmATTACH_FORM, XmNalignment,
        XmALIGNMENT_BEGINNING, XmNrecomputeSize, FALSE, XmNlabelString, xms, NULL);
    XmStringFree(xms);
    ...
    canvasFrame = XtVaCreateManagedWidget(
        "canvasFrame", xmScrolledWindowWidgetClass, form, XmNtopAttachment, XmATTACH_WIDGET,
        XmNtopWidget, titleFrame, XmNbottomAttachment, XmATTACH_WIDGET, XmNbottomWidget, helpLabel,
        XmNleftAttachment, XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM, XmNshadowThickness, 1,
        XmNwidth, 400, XmNheight, 250, NULL);

    setHelp(canvasFrame, helpLabel, "Double click on a document or folder to open it.");
    canvas = XtVaCreateManagedWidget("canvas", widgetClass, canvasFrame, XmNwidth, 1, XmNheight, 1,
