...
static MenuItem fileMenuItems[] = {
    {"Open File", &xmPushButtonWidgetClass, 'O', "Ctrl<Key>O", "Ctrl-O", openDocument, "Open",
    ...
    {"Reload", &xmPushButtonWidgetClass, 'R', "Ctrl<Key>R", "Ctrl-R", oneMessageCB, "reload",
     "Reload the current document.", TRUE, (MenuItem*)NULL},
    {"Clone", &xmPushButtonWidgetClass, 'C', "Ctrl<Key>C", "Ctrl-C", clonePageCB, "clone",
     "Clone the current document.", TRUE, (MenuItem*)NULL},
    /*
...
static MenuItem navigationMenuItems[] = {
    ...
    {"History List", &xmPushButtonWidgetClass, 'L', "Ctrl<Key>L", "Ctrl-L", showHistoryCB,
     "History List", "Show history of document link traversals.", TRUE, (MenuItem*)NULL},
    {"", &xmSeparatorGadgetClass, (char)0, NULL, NULL, NULL, NULL, NULL, TRUE, (MenuItem*)NULL},
...
static MenuItem fontsMenuItems[] = {
    {"Helvetica small", &xmPushButtonWidgetClass, (char)0, NULL, NULL, changeFonts,
     "useHelveticaSmallFonts", "Pick a font", TRUE, (MenuItem*)NULL},
    {"Helvetica medium", &xmPushButtonWidgetClass, (char)0, NULL, NULL, changeFonts,
     "useHelveticaMediumFonts", "Pick a font", TRUE, (MenuItem*)NULL},
    {"Helvetica large", &xmPushButtonWidgetClass, (char)0, NULL, NULL, changeFonts,
     "useHelveticaLargeFonts", "Pick a font", TRUE, (MenuItem*)NULL},
    {"", &xmSeparatorGadgetClass, (char)0, NULL, NULL, NULL, NULL, NULL, TRUE, (MenuItem*)NULL},
    {"Times small", &xmPushButtonWidgetClass, (char)0, NULL, NULL, changeFonts,
     "useTimesSmallFonts", "Pick a font", TRUE, (MenuItem*)NULL},
    {"Times medium", &xmPushButtonWidgetClass, (char)0, NULL, NULL, changeFonts,
     "useTimesMediumFonts", "Pick a font", TRUE, (MenuItem*)NULL},
    {"Times large", &xmPushButtonWidgetClass, (char)0, NULL, NULL, changeFonts,
     "useTimesLargeFonts", "Pick a font", TRUE, (MenuItem*)NULL},
    {"", &xmSeparatorGadgetClass, (char)0, NULL, NULL, NULL, NULL, NULL, TRUE, (MenuItem*)NULL},
    {"Fixed", &xmPushButtonWidgetClass, (char)0, NULL, NULL,
     /*'F', "Ctrl<Key>F",  "Ctrl-F",*/
     changeFonts, "useAllFixedFonts", "A very simple font, in case other fonts don't work.", TRUE,
     (MenuItem*)NULL},
    NULL,
};
...
int main(int argc, char* argv[])
{
    ...
    topWidget = XtVaAppInitialize(&appCon, "Viola", NULL, 0, &argc, argv, fallback_resources,
                                  XmNborderWidth, 2, NULL);
...
DocViewInfo* makeBrowserInterface(Widget shell, char* shellName, DocViewInfo* parentInfo, int argc, char* argv[])
{
    ...
    form = XtVaCreateManagedWidget("form", xmFormWidgetClass, mainWin, XmNhorizontalSpacing,
    ...
    titleIconFrame = XtVaCreateManagedWidget(
        "titleIconFrame", xmFrameWidgetClass, topForm, XmNtopAttachment, XmATTACH_FORM,
    ...
    titleIcon = XtVaCreateManagedWidget("titleIcon", xmLabelWidgetClass, titleIconFrame,
                                        XmNlabelType, XmPIXMAP, XmNrecomputeSize, FALSE, XmNwidth,
                                        TITLE_ICON_WIDTH, XmNheight, TITLE_ICON_HEIGHT,
                                        XmNbackground, XBlackPixelOfScreen(XtScreen(shell)), NULL);
    docViewInfo->titleIcon = titleIcon;
    docViewInfo->intervalID =
        XtAppAddTimeOut(appCon, GLOBE_TIME, flipGlobe, (XtPointer)docViewInfo);
    XtAddEventHandler(titleIcon, ButtonReleaseMask, FALSE, navigateHomeEH, (XtPointer)docViewInfo);

    titleFrame = XtVaCreateManagedWidget(
        "titleFrame", xmFrameWidgetClass, topForm, XmNtopAttachment, XmATTACH_FORM,
        XmNbottomAttachment, XmATTACH_FORM, XmNleftAttachment, XmATTACH_WIDGET, XmNleftWidget,
        titleIconFrame, XmNleftOffset, 4, XmNrightAttachment, XmATTACH_FORM, XmNmarginWidth, 4,
    ...
    canvasForm = XtVaCreateManagedWidget("canvasForm", xmFormWidgetClass, frame,
                                         XmNhorizontalSpacing, 1, XmNverticalSpacing, 1, NULL);

    scrollBar = XtVaCreateManagedWidget("scrollBar", xmScrollBarWidgetClass, canvasForm,
                                        XmNtopAttachment, XmATTACH_FORM, XmNbottomAttachment,
                                        XmATTACH_FORM, XmNrightAttachment, XmATTACH_FORM,
                                        XmNmaximum, SBAR_MAGNITUDE, NULL);

    violaCanvas = XtVaCreateManagedWidget(
        "violaCanvas", xmPrimitiveWidgetClass, canvasForm, XmNtopAttachment, XmATTACH_FORM,
    ...
    /* Menubar. */
    menuBar = buildMenus(mainMenus, helpMenuItems, mainWin, messageText, docViewInfo);
    XmMainWindowSetAreas(mainWin, menuBar, (Widget)NULL, (Widget)NULL, (Widget)NULL, form);
    ...
    if (shell == topWidget) {
        ...
        status = initViola(argc, argv, VIOLA_OBJ_NAME, XtDisplay(violaCanvas),
                           XtScreen(violaCanvas), XtWindow(violaCanvas));
        if (status == 0) {
            exit(100);
        }
        ...
        docViewInfo->violaDocViewObj = findObject(getIdent(docViewInfo->violaDocViewName));
        docViewInfo->violaDocToolsObj = findObject(getIdent(docViewInfo->violaDocToolsName));

        if (!(docViewInfo->violaDocViewObj && docViewInfo->violaDocToolsObj)) {
            exit(101);
        }
        docViewInfo->violaDocViewWindow = GET_window(docViewInfo->violaDocViewObj);
        docViewInfo->violaDocToolsWindow = GET_window(docViewInfo->violaDocToolsObj);

        docViewInfo->cloneID = (long)0;
        XtAppAddWorkProc(appCon, (XtWorkProc)doViolaIdle, (XtPointer)NULL);

        XtVaGetValues(violaCanvas, XmNwidth, &width, XmNheight, &height, NULL);

        XResizeWindow(XtDisplay(violaCanvas), docViewInfo->violaDocViewWindow, (unsigned int)width,
                      (unsigned int)height);
        XMoveWindow(XtDisplay(violaCanvas), docViewInfo->violaDocViewWindow, -1, -1);
        XtAddEventHandler(violaCanvas, StructureNotifyMask, FALSE, resizeViola,
        ...
        */
        ...
        XResizeWindow(XtDisplay(toolBarForm), docViewInfo->violaDocToolsWindow, width, height);
        XtAddEventHandler(toolBarForm, StructureNotifyMask, FALSE, resizeViola,
                          (XtPointer)docViewInfo->violaDocToolsWindow);

        XtAddCallback(scrollBar, XmNdragCallback, scrollBarDrag,
                      (XtPointer)docViewInfo->violaDocViewObj);
        XtAddCallback(scrollBar, XmNvalueChangedCallback, scrollBarValueChanged,
                      (XtPointer)docViewInfo->violaDocViewObj);
        ...
        /*	ViolaRegisterMessageHandler("replaceHotlistItem", replaceHotlistItemMH, (void
         * *)docViewInfo);
         */
    }
...
void setHelp(Widget widget, Widget helpLabel, char* helpText)
{
    ...
    if (helpText) {
        ...
        XtAddEventHandler(widget, EnterWindowMask, FALSE, helpHandler, (XtPointer)hdata);
        XtAddEventHandler(widget, LeaveWindowMask, FALSE, blankHelpHandler, (XtPointer)hdata);
    } else {
        hdata->helpText = "";
        XtAddEventHandler(widget, EnterWindowMask, FALSE, helpHandler, (XtPointer)hdata);
        XtAddEventHandler(widget, LeaveWindowMask, FALSE, blankHelpHandler, (XtPointer)hdata);
...
XmString makeXMSTitle(char * title, char * subTitle)
{
    ...
    if (subTitle && strlen(subTitle)) {
        titleXMS = XmStringSegmentCreate(title, "titleFont", XmSTRING_DIRECTION_L_TO_R,
                                         (strlen(subTitle) ? TRUE : FALSE));
        subTitleXMS =
            XmStringSegmentCreate(subTitle, "subTitleFont", XmSTRING_DIRECTION_L_TO_R, FALSE);
