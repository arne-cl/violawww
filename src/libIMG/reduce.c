...
#define TRUE_TO_15BIT(PIXEL)                                                                       \
    ((((PIXEL) & 0xf80000) >> 9) | (((PIXEL) & 0x00f800) >> 6) | (((PIXEL) & 0x0000f8) >> 3))
...
struct color_area **rlargest, **rsmallest, *area;
{
     ...
     */
        ...
        for (tmp_area = largest; tmp_area; tmp_area = tmp_area->next)
            if ((area->pixel_count > tmp_area->pixel_count) || (tmp_area->num_pixels < 2)) {
...
unsigned int n, verbose;
{
    ...
    case IRGB:
        ...
        for (y = 0; y < image->height; y++)
            for (x = 0; x < image->width; x++) {
                pixel_counts[TLA_TO_15BIT(image->rgb, memToVal(pixel, image->pixlen))]++;
                pixel += image->pixlen;
            }
        break;
    ...
    case ITRUE:
        ...
        if (verbose) {
            printf("  Converting true color image to RGB image with %d colors...", n);
            fflush(stdout);
        }
    ...
    case IRGB:
        for (y = 0; y < image->height; y++)
            for (x = 0; x < image->width; x++) {
                valToMem(pixel_counts[TLA_TO_15BIT(image->rgb, memToVal(pixel, image->pixlen))],
    ...
    case ITRUE:
        for (y = 0; y < image->height; y++)
            for (x = 0; x < image->width; x++) {
                valToMem(pixel_counts[TRUE_TO_15BIT(memToVal(pixel, 3))], dpixel,
                         new_image->pixlen);
                pixel += 3;
                dpixel += new_image->pixlen;
            }
    ...
    case IRGB:
        ...
        for (y = 0; y < image->height; y++)
            for (x = 0; x < image->width; x++) {
                ...
                valToMem(RGB_TO_TRUE(image->rgb.red[spixval], image->rgb.green[spixval],
