...
Image* rleLoad(char* fullname, char* name, unsigned int verbose)
{
    ...
    if (ptype ==
        SC_M) { /* don't mess with the image, but change their colour map a bit if needed */
    ...
    scan = (unsigned char**)lmalloc((sv_globals.sv_ncolors + sv_globals.sv_alpha) *
    ...
    if (!bwflag && ptype == SC_M) {
        ...
        for (i = 0; i < ncol; i++) {
            *(image->rgb.red + i) = *(image->rgb.green + i) = *(image->rgb.blue + i) = gammamap[i]
                                                                                       << 8;
        }
    } else {
...
void c_m_line(unsigned char* dp, int number, int line)
{
    ...
    if (!bwflag) {
        ...
        int red, green, blue;
        for (i = number, r = &scan[0][0], g = &scan[1][0], b = &scan[2][0]; i > 0;
             i--, r++, g++, b++, dp++) {
            red = fmaps[0][*r];
            green = fmaps[1][*g];
            blue = fmaps[2][*b];
