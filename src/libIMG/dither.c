...
Image* dither(Image* cimage, unsigned int verbose) {
    ...
    if (RGBP(cimage) && (cimage->depth <= 16)) {
        ...
        for (i = 0; i < cimage->rgb.used; i++)
            grey[i] =
                (colorIntensity(cimage->rgb.red[i], cimage->rgb.green[i], cimage->rgb.blue[i]) >>
                 1);
    ...
    for (i = 0; i < cimage->height; i++) {
        ...
        for (j = 0; j < cimage->width; j++) {
            ...
            if (RGBP(cimage)) {
                if (grey == NULL)
                    level = tone_scale_adjust(colorIntensity(cimage->rgb.red[color],
                ...
                level = tone_scale_adjust(colorIntensity((TRUE_RED(color) << 8),
                                                         (TRUE_GREEN(color) << 8),
                                                         (TRUE_BLUE(color) << 8)) >>
                                          1);
            }
...
Image* approx(Image* cimage, unsigned int verbose) {
    ...
    printf("Colors used:\n");
    for (i = 0; i < cimage->rgb.used; i++) {
        printf("[%d] r=%d g=%d b=%d\n", i, cimage->rgb.red[i], cimage->rgb.green[i],
               cimage->rgb.blue[i]);
        /*
...
static unsigned int tone_scale_adjust(unsigned int val) {
    ...
    if (val < Threshold)
        ...
        rslt = (((val - Threshold) * (MaxGrey - (Threshold / 2))) / (MaxGrey - Threshold)) +
               (Threshold / 2);
