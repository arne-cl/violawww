...
struct sv_dispatch_tab {
    ...
    sv_fn *setup, /* startup function */
        *skipBlankLines, *setColor, *skipPixels, *newScanLine,
...
struct sv_dispatch_tab sv_DTable[] = {
    {" OB", RunSetup, RunSkipBlankLines, RunSetColor, RunSkipPixels, RunNewScanLine, Runputdata,
     Runputrun, DefaultBlockHook, RunputEof},
};
...
/* short instructions */
#define mk_short_1(oper, a1) /* one argument short */ putc(oper, sv_fd), putc((char)a1, sv_fd)
...
#define mk_long_2(oper, a1, a2) /* two argument long */                                            \
    putc((char)(LONG | oper), sv_fd), putc('\0', sv_fd), put16(a1), put16(a2)
...
#define mk_inst_2(oper, a1, a2) /* two argument inst */                                            \
    if (a1 > UPPER)                                                                                \
        (mk_long_2(oper, a1, a2));                                                                 \
    else                                                                                           \
        (mk_short_2(oper, a1, a2))
...
#define BREAD(type, var, len) zread(infile, (byte*)&var, len)
...
int rle_get_setup(struct sv_globals* globals) {
    ...
    if (!(setup.h_flags & H_NO_BACKGROUND)) {
        globals->sv_bg_color = (int*)lmalloc((unsigned)(sizeof(int) * setup.h_ncolors));
        bg_color = (rle_pixel*)lmalloc((unsigned)(1 + (setup.h_ncolors / 2) * 2));
    ...
    if (globals->sv_ncmap > 0) {
        int maplen = globals->sv_ncmap * (1 << globals->sv_cmaplen);
        globals->sv_cmap = (rle_map*)lmalloc((unsigned)(sizeof(rle_map) * maplen));
        if (globals->sv_cmap == NULL) {
            fprintf(stderr, "Malloc failed for color map of size %d*%d in rle_get_setup\n",
    ...
    if (setup.h_flags & H_COMMENT) {
        ...
        if (comment_buf == NULL) {
            fprintf(stderr, "Malloc failed for comment buffer of size %d in rle_get_setup\n",
                    comlen);
            return -2;
        }
        ...
        if (globals->sv_comments == NULL) {
            fprintf(stderr, "Malloc failed for %d comment pointers in rle_get_setup\n", i);
            return -2;
        }
...
int rle_get_error(int code, char* pgmname, char* fname)
{
    ...
    case -2: /* malloc failed */
        fprintf(stderr, "%s: Malloc failed reading header of file %s\n", pgmname, fname);
        break;
    ...
    case -4:
        fprintf(stderr, "%s: RLE header of %s is incomplete (premature EOF)\n", pgmname, fname);
        break;
...
int rle_getrow(struct sv_globals* globals, rle_pixel* scanline[])
{
    ...
    if (globals->sv_background == 2) {
        ...
        for (nc = 0; nc < globals->sv_ncolors; nc++)
            if (SV_BIT(*globals, nc))
                bfill((char*)scanline[nc], globals->sv_xmax + 1, globals->sv_bg_color[nc]);
    }
    ...
    for (;;) {
        ...
        default:
            fprintf(stderr, "rle_getrow: Unrecognized opcode: %d\n", inst.opcode);
            exit(1);
        }
...
void make_square(double N, int divN[256], int modN[256], int magic[16][16])
{
    ...
    if (dith_np2)
        ...
        for (dith_size = 2; ((dith_size * dith_size) + 1) < ((N * dith_levels) / 256);
             dith_size *= 2)
            ;
...
int dithergb(int x, int y, int r, int g, int b, int levels, int divN[256], int modN[256], int magic[16][16])
{
    ...
    return DMAP(r, col, row) + DMAP(g, col, row) * levels + DMAP(b, col, row) * levels * levels;
}
