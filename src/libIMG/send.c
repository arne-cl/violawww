...
Pixmap ximageToPixmap(Display* disp, Window parent, XImageInfo* ximageinfo)
{
    ...
    XSync(disp, False);
    pixmap = XCreatePixmap(disp, parent, ximageinfo->ximage->width, ximageinfo->ximage->height,
                           ximageinfo->depth);
    (void)XSetErrorHandler(old_handler);
    if (GotError)
        return (None);
    ximageinfo->drawable = pixmap;
    sendXImage(ximageinfo, 0, 0, 0, 0, ximageinfo->ximage->width, ximageinfo->ximage->height);
    return (pixmap);
}
...
/* ddepth: depth of the visual to use */
{
    ...
    retry_direct: /* tag we hit if a DirectColor allocation fails on
        ...
        if ((redcolors > visual->map_entries) || (greencolors > visual->map_entries) ||
...
Warning: inconsistency in color information (this may be ugly)\n");
        ...
        for (a = 0; a < visual->map_entries; a++) {
            ...
            if (!XAllocColor(disp, ximageinfo->cmap, &xcolor)) {
                ...
                if ((visual->class == DirectColor) && (visual == DefaultVisual(disp, scrn))) {
                    ximageinfo->cmap =
...
imageToXImage: XAllocColor failed on a TrueColor/Directcolor visual\n");
    ...
    retry: /* this tag is used when retrying because we couldn't get a fit */
        ...
        if (private_cmap) { /* user asked us to use a private cmap */
            newmap = 1;
            fit = 0;
        } else if ((visual == DefaultVisual(disp, scrn)) || (visual->class == StaticGray) ||
        ...
        if (newmap) {
            ...
            for (a = 0; a < image->rgb.used; a++) /* count entries we got */
                if (!XAllocColorCells(disp, ximageinfo->cmap, False, NULL, 0, index + a, 1))
                    break;
#endif
            if (fit) {
                /*	if (a > 0)
                        ...
                        if (close > 150) {
                            ...
                            */
                            ...
                            /*
                                          ...
                                          printf("no close match, try allocating %d,%d,%d (%d %d
                            ...
                            */
                            ...
                            if (XAllocColor(disp, theCmap, &xcolor)) {
                                ...
                                */
                                ...
                                /*		printf("simply can't do it!!! Using closest
                                 * color.\n");*/
                            ...
                            */
                            /*	      printf("approximating %d,%d,%d to %d,%d,%d\n",
                                                 r, g, b,
                                                 ctab[close].red, ctab[close].green,
                               ctab[close].blue);
                            */
                            *(index + i) = ctab[close].pixel;
                        }
    ...
    case ITRUE:
        ...
        default:
            ...
            if (dbits % 8) {
                ...
                ximageinfo->ximage = XCreateImage(disp, visual, ddepth, XYPixmap, 0, NULL,
...
void sendXImage(XImageInfo* ximageinfo, int src_x, int src_y, int dst_x, int dst_y, unsigned int w, unsigned int h)
{
    ...
    XPutImage(ximageinfo->disp, ximageinfo->drawable, ximageinfo->gc, ximageinfo->ximage, src_x,
              src_y, dst_x, dst_y, w, h);
}
