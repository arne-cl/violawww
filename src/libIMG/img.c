...
unsigned int imgIdent(char* fullname, char* name)
{
    ...
    if (IMG_ReadHeader(zf, &header) == 0) {
        printf("%s is a %dx%d GEM Bit Image\n", fullname, header.llen, header.lines);
        ret = 1;
    }
...
static int IMG_ReadHeader(ZFILE* zf, IMG_Header* h) {
    ...
    if ((h->vers != DEF_VERSION) || (h->hlen < DEF_HLEN) || (h->colors < 0) || (h->colors > 256) ||
        (h->pixw < 1) || (h->pixh < 1) || (h->llen < 1) || (h->llen > (MAX_SCANLINE * 8)) ||
        (h->lines < 1) || (h->lines > 8192) || (h->patlen < 0) || (h->patlen > MAX_LITERAL))
        return (2);
