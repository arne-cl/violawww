...
int *width, *height;
{
    ...
    for (a = 0; a < imagecount; a++) {
        ...
        if ((a == 0) && onroot && fullscreen && (images[0].xzoom == 0.0) &&
            (images[0].yzoom == 0.0) && (images[0].atx == 0) && (images[0].aty == 0) &&
            (images[0].center == 0)) {
        ...
        if (dispimage) {
            ...
            tmpimage = merge(dispimage, newimage, images[a].atx, images[a].aty, verbose);
        ...
        *ximageinfo =
            imageInWindow(disp, scrn, dispimage, user_geometry, fullscreen, install, private_cmap,
                          fit, use_pixmap, images[a].delay, visual_class, argc, argv, verbose);
