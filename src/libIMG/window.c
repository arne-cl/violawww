...
static int getNextEventWithTimeout(Display* disp, XEvent* event) {
    ...
    while ((AlarmWentOff == 0)) {
        ...
        nfound = select(ConnectionNumber(disp) + 1, &rmask, (fd_set*)0, (fd_set*)0,
                        /*(struct timeval *)*/ 0);
        switch (nfound) {
        case -1:
            if (errno == EINTR) {
...
int x, y, w, h;
{
    ...
    if (x + w > width) {
        XClearArea(ximageinfo->disp, ximageinfo->drawable, x + width, y, x + w - width, h, False);
        w -= x + w - width;
    }
    if (y + h > height) {
        XClearArea(ximageinfo->disp, ximageinfo->drawable, x, y + height, w, y + h - height, False);
        h -= y + h - height;
    }
...
} VisualClassName[] = {TrueColor,   "TrueColor",   DirectColor, "DirectColor",
                       PseudoColor, "PseudoColor", StaticColor, "StaticColor",
...
int visualClassFromName(char* name) {
    ...
    for (a = 0; VisualClassName[a].name; a++) {
        for (s1 = VisualClassName[a].name, s2 = name; *s1 && *s2; s1++, s2++)
            if ((isupper(*s1) ? tolower(*s1) : *s1) != (isupper(*s2) ? tolower(*s2) : *s2))
                break;
...
unsigned int depth;
{
    ...
    if (!(info = XGetVisualInfo(disp, VisualScreenMask | VisualClassMask | VisualDepthMask,
                                &template, &nvisuals)))
...
unsigned int* rdepth;
{
    ...
    for (a = 0; a < screen->ndepths; a++) {
        if (screen->depths[a].nvisuals &&
            ((!depth || ((depth < image->depth) && (screen->depths[a].depth > depth)) ||
              ((screen->depths[a].depth >= image->depth) && (screen->depths[a].depth < depth)))))
    ...
    case ITRUE:
        ...
        if (!visual || ((depth <= 8) && bestVisualOfClassAndDepth(disp, scrn, PseudoColor, depth)))
...
unsigned int verbose;
{
    ...
    if (user_geometry == NULL) {
        ...
        XGeometry(disp, scrn, user_geometry, def_geom, 0, 1, 1, 0, 0, &winx, &winy, &winwidth,
                  &winheight);
    }
        ...
        if (visual_class == -1) {
            ...
            if (!visual) {
                ...
                fprintf(stderr, "Server does not support %s visual, using %s\n",
                        nameOfVisualClass(visual_class), nameOfVisualClass(visual->class));
            }
        }
    }
    ...
    if (!(ximageinfo =
              imageToXImage(disp, scrn, visual, depth, image, private_cmap, fit, verbose))) {
    ...
    if (!ViewportWin) {
        ViewportWin = XCreateWindow(
            disp, RootWindow(disp, scrn), winx, winy, winwidth, winheight, 0,
            DefaultDepth(disp, scrn), InputOutput, DefaultVisual(disp, scrn),
            CWBackingStore | CWBackPixel | CWCursor | CWEventMask | CWSaveUnder, &swa_view);
        oldimagewindow = 0;
    ...
    ImageWindow = XCreateWindow(disp, ViewportWin, winx, winy, image->width, image->height, 0,
                                ximageinfo->depth, InputOutput, visual,
                                CWBitGravity | CWColormap | CWSaveUnder | CWBorderPixel, &swa_img);
    ...
    if ((DoesBackingStore(ScreenOfDisplay(disp, scrn)) == NotUseful) || use_pixmap) {
        if (((pixmap = ximageToPixmap(disp, ImageWindow, ximageinfo)) == None) && verbose)
    ...
    setCursor(disp, ViewportWin, image->width, image->height, winwidth, winheight,
              &(swa_view.cursor));
    lastx = lasty = -1;
    if (delay) {
    ...
    for (;;) {
        ...
        case ClientMessage:
            ...
            if ((event.message.window == ViewportWin) && (event.message.data.l[0] == delete_atom)) {
                cleanUpImage(disp, scrn, swa_view.cursor, pixmap, image, ximageinfo);
                return (NULL);
            }
            break;
