...
static int isXWD(char* name, ZFILE* zf, XWDHeader* header, int verbose) {
    ...
    if (verbose) {
        printf("%s is a %dx%d XWD image in ", name, header->pixmap_width, header->pixmap_height);
        switch (header->pixmap_format) {
        case XYBitmap:
            printf("XYBitmap");
            break;
...
XWDHeader header;
{
    ...
    xunits = (header.pixmap_width / (unit * 8)) + (header.pixmap_width % (unit * 8) ? 1 : 0);
    ...
    for (y = 0; y < header.pixmap_height; y++) {
        if (zread(zf, (byte*)line, dlinelen) != dlinelen) {
            fprintf(stderr, "%s: Short read while reading data! (returning partial image)\n",
                    fullname);
            lfree(line);
            return (image);
        }
        ...
        for (y = 0; y < header.pixmap_height; y++) {
            ...
            for (x = 0; x < xunits; x++) {
                ...
                for (x2 = 0; x2 < unitbits; x2++) {
                    if (pixvals & mask)
                        valToMem(memToVal(iptr + index, image->pixlen) | (1 << plane), iptr + index,
                                 image->pixlen);
                    index += image->pixlen;
                    if (index > ilinelen) {
                        x = xunits;
    ...
    for (y = 0; y < header.pixmap_height; y++) {
        ...
        for (x = 0; x < header.pixmap_width; x++) {
            ...
            case 32:
                ...
                break;
            default:
                fprintf(stderr, "%s: ZPixmaps of depth %d are not supported (sorry).\n", fullname,
                        header.pixmap_depth);
                exit(1);
            }
        }
...
Image* xwdLoad(char* fullname, char* name, int verbose) {
    ...
    if (header.pixmap_depth > 16) {
        fprintf(stderr, "%s: Sorry, cannot load images deeper than 16 bits (yet)\n", fullname);
        exit(1);
    }
