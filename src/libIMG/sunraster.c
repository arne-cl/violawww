...
static void babble(char* name, struct rheader* header)
{
    ...
    case 1:
        ...
        break;
    case 8:
        printf(" 8 plane %s", memToVal(header->maplen, 4) > 0 ? "color" : "greyscale");
        break;
    case 24:
...
Image* sunRasterLoad(char* fullname, char* name, unsigned int verbose)
{
    ...
    switch (depth) {
    case 1:
        image = newBitImage(memToVal(header.width, 4), memToVal(header.height, 4));
        break;
    case 8:
        image = newRGBImage(memToVal(header.width, 4), memToVal(header.height, 4),
