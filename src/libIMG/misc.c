...
void memoryExhausted() {
    fprintf(stderr, "Memory has been exhausted; operation cannot continue (sorry).\n");
    if (_Xdebug)
        abort();
    else
        exit(1);
...
unsigned int verbose;
{
    ...
    if ((options->clipx != 0) || (options->clipy != 0) || (options->clipw != 0) ||
        (options->cliph != 0)) {
        if (!options->clipw)
        ...
        tmpimage = clip(image, options->clipx, options->clipy,
                        ...
                        (options->cliph ? options->cliph : image->height), verbose);
        freeImage(image);
        image = tmpimage;
    }
