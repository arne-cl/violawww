...
04/13/91   5 nazgul	Bug fix to retry with bitreversed, and do not double allocate on multiple
calls 04/12/91   4 nazgul	Spot faxes that do not have a 000 header Handle faxes that have the
bytes in the wrong order 07/03/90   2 nazgul	Added recovery for premature EOF
*/
    ...
    while (!g3_eof) {
        ...
        if (curlen > 13) {
            ...
            if (g3_verb)
                fprintf(
                    stderr,
                    "G3: Bad code word at row %d, col %d (len %d code 0x%2.2x), skipping to EOL.\n",
...
tryagain:
    ...
    bufptr = fd->bufptr;
    if (!g3_error)
        col1 = g3_getfaxrow(fd, tmpline);
    if (!g3_error)
        col2 = g3_getfaxrow(fd, tmpline);
    if (!g3_error && col1 == col2 && col1 != 0)
        ret = 1;
    else
        ret = 0;
    /* if (ret) printf("%d = %d\n", col1, col2); */
