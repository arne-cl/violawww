...
#define MAXPRINTABLE                                                                               \
    93 /* number of printable ascii chars                                                          \
...
LFUNC(GetImagePixels, int,
      (XImage * image, unsigned int width, unsigned int height, PixelsMap* pmap));

LFUNC(GetImagePixels32, int,
      (XImage * image, unsigned int width, unsigned int height, PixelsMap* pmap));

LFUNC(GetImagePixels16, int,
      (XImage * image, unsigned int width, unsigned int height, PixelsMap* pmap));

LFUNC(GetImagePixels8, int,
      (XImage * image, unsigned int width, unsigned int height, PixelsMap* pmap));
...
int xpmScanImage(Display* display, XImage* image, XImage* shapeimage, XpmAttributes* attributes, xpmInternAttrib* attrib)
{
    ...
    if (attributes &&
        (attributes->valuemask & XpmCharsPerPixel || attributes->valuemask & XpmInfos))
        cpp = attributes->cpp;
    else
        cpp = 0;

    pmap.pixelindex = (unsigned int*)calloc(width * height, sizeof(unsigned int));
    if (!pmap.pixelindex)
        RETURN(XpmNoMemory);
     ...
     */
    if (shapeimage) {
        ErrorStatus = GetImagePixels1(shapeimage, width, height, &pmap, storeMaskPixel);
    ...
    if (image) {
        if (image->depth == 1)
            ErrorStatus = GetImagePixels1(image, width, height, &pmap, storePixel);
    ...
    if (*((char*)&byteorderpixel) == image->byte_order) {
        ...
        for (y = 0; y < height; y++)
            for (x = 0; x < width; x++, iptr++) {
                ...
                pixel = ((unsigned long)addr[0] << 24 | (unsigned long)addr[1] << 16 |
        ...
        for (y = 0; y < height; y++)
            for (x = 0; x < width; x++, iptr++) {
                addr = &((unsigned char*)image->data)[ZINDEX32(x, y, image)];
                pixel = ((unsigned long)addr[3] << 24 | (unsigned long)addr[2] << 16 |
