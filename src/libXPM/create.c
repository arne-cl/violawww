...
LFUNC(SetImagePixels, int,
      (XImage * image, unsigned int width, unsigned int height, unsigned int* pixelindex,
       Pixel* pixels));

LFUNC(SetImagePixels32, int,
      (XImage * image, unsigned int width, unsigned int height, unsigned int* pixelindex,
       Pixel* pixels));

LFUNC(SetImagePixels16, void,
      (XImage * image, unsigned int width, unsigned int height, unsigned int* pixelindex,
       Pixel* pixels));

LFUNC(SetImagePixels8, void,
      (XImage * image, unsigned int width, unsigned int height, unsigned int* pixelindex,
       Pixel* pixels));

LFUNC(SetImagePixels1, void,
      (XImage * image, unsigned int width, unsigned int height, unsigned int* pixelindex,
       Pixel* pixels));
...
XpmAttributes* attributes;
{
    ...
    if (image_return) {
        ...
        if (image->depth == 1)
            ...
            SetImagePixels16(image, attrib->width, attrib->height, attrib->pixelindex,
                             image_pixels);
        else if (image->bits_per_pixel == 32)
            SetImagePixels32(image, attrib->width, attrib->height, attrib->pixelindex,
                             image_pixels);
        else
     ...
     */
    if (attributes &&
        (attributes->valuemask & XpmReturnInfos || attributes->valuemask & XpmReturnPixels)) {
...
XImage** image_return;
{
    ...
    *image_return =
        XCreateImage(display, visual, depth, ZPixmap, 0, 0, width, height, bitmap_pad, 0);
    if (!*image_return)
        return (XpmNoMemory);
...
LFUNC(_putbits, int, (char* src, int dstoffset, int numbits, char* dst));
        ...
        for (y = 0; y < height; y++)
            for (x = 0; x < width; x++, iptr++) {
                ...
                for (i = nbytes; --i >= 0;)
                    *dst++ = *src++;
                ZNORMALIZE(&px, image);
                _putbits((char*)&pixel, (x * image->bits_per_pixel) & 7, image->bits_per_pixel,
                         (char*)&px);
                ZNORMALIZE(&px, image);
                src = (char*)&px;
...
static void SetImagePixels16(XImage* image, unsigned int width, unsigned int height,
...
static void SetImagePixels8(XImage* image, unsigned int width, unsigned int height,
                            unsigned int* pixelindex, Pixel* pixels) {
    unsigned int* iptr;
    int x, y;
...
static void SetImagePixels1(XImage* image, unsigned int width, unsigned int height,
                            unsigned int* pixelindex, Pixel* pixels) {
    unsigned char bit;
    int xoff, yoff;
    unsigned int* iptr;
