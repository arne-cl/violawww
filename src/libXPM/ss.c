...
main() {
    ...
    */
    ...
    w = XCreateWindow(display, RootWindow(display, screen_num), (int)10, (int)10, (int)200,
                      (int)100, (int)1, /* border thickness */
                      CopyFromParent, CopyFromParent, CopyFromParent,
                      CWBackPixel | CWBorderPixel | CWEventMask | CWCursor, &attrs);
...
");
    ...
    ErrorStatus =
        XpmReadFileToPixmap(display, w, bitmapfd, &view.pixmap, &view.mask, &view.attributes);
    ...
    if (!gc_dash) {
        ...
        gc_dash = XCreateGC(display, w,
                            GCForeground | GCBackground | GCDashOffset | GCDashList | GCLineStyle |
                                GCSubwindowMode,
                            &values);
        ...
        XChangeGC(display, gc_dash, GCForeground | GCBackground | GCLineStyle, &gcv);
...
processEvent(event) XEvent* event;
{
    int x, y;
