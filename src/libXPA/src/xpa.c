...
typedef enum xpa_type_defn { PANE_T, ITEM_T, PB_T, TITLE_T } xpa_type;
 ...
 */
{
    ...
    if (!(new_context = XPA_MEM(xpa_context)))
        xpa_raise(XPA_NOMEM);
 ...
 */
{
    if (intern->state[item->level].item == item) {
        ...
        */
        XDrawRectangle(intern->disp, item->item_win,
                       cr_gc(intern->disp, item->item_win, intern->attr->cur_fg), 0, 0,
                       item->iw - 1, item->ih - 1);
        ...
        /* Not selected */
        XFillRectangle(intern->disp, item->item_win,
                       bg_gc(intern->disp, item->item_win, intern->attr->item_bg), 0, 0, item->iw,
                       item->ih);
        if (item->key_buf[0]) {
            XDrawString(
                intern->disp, item->item_win,
 ...
 */
{
    ...
    if (intern->state[pb->item->level].item == pb->item) {
        ...
        */
        XDrawString(intern->disp, pb->pb_win,
                    fg_gc(intern->disp, pb->pb_win, pb_sym_font, intern->attr->item_fg), ox, oy, ch,
                    1);
    }
}
 ...
 */
{
    if (data->any.type != PB_T)
        xpa_raise(XPA_TYPE);
    switch (evt->type) {
    case EnterNotify:
        /* Post subpane */
        if (evt->xcrossing.detail == NotifyAncestor) {
            pane_on(intern, data->pb.item->sub_pane, data->pb.item->level + 1,
                    intern->state[data->pb.item->level].x + data->pb.item->iw - XPA_HORPAD -
                        XPA_PB_W,
                    intern->state[data->pb.item->level].y + data->pb.item->fromtop, (xpa_item*)0);
        }
        break;
    case Expose:
        if (evt->xexpose.count == 0) {
 ...
 */
{
    ...
    attr.cursor = cur;
    wamask =
        CWBackPixel | CWBorderPixel | CWSaveUnder | CWOverrideRedirect | CWEventMask | CWCursor;

    return XCreateWindow(disp, parent, x, y, width, height, bwidth, CopyFromParent, InputOutput,
 ...
 */
{
    ...
    *rw = 2 * XPA_HORPAD + XTextWidth(font, name, strlen(name));
    ...
    largest = 0;
    if (font->ascent > largest)
        largest = font->ascent;
    if (ac_font->ascent > largest)
        largest = ac_font->ascent;
    if (XPA_PB_H > largest)
        largest = XPA_PB_H;
    *basel = largest + XPA_VERPAD;

    largest = 0;
    if (font->descent > largest)
        largest = font->descent;
    if (ac_font->descent > largest)
        largest = ac_font->descent;
 ...
 */
{
    ...
    pullbox =
        make_window(intern->disp, parent, x, 1, XPA_PB_W, height, XPA_PB_B, intern->attr->border_bd,
                    intern->attr->item_bg, (Cursor)None, EnterWindowMask | ExposureMask, 0);
 ...
 */
{
    ...
    if (entry->item_type == XPA_ITEM_TYPE_ENTRY) {
        ...
        item_size(intern->attr->item_font, entry->item_name, (entry->sub_entrys != (xpa_entry*)0),
                  intern->attr->key_font, entry->key_char, &exp_w, &exp_h, &bl, &off);
        item_win =
            make_window(intern->disp, parent, 0, yspot, width, exp_h, 0, intern->attr->border_bd,
                        intern->attr->item_bg, (Cursor)None, EnterWindowMask | ExposureMask, 0);
        ...
        */
        ...
        exp_h = 1;
        item_win =
            make_window(intern->disp, parent, 0, yspot, width, exp_h, 1, intern->attr->border_bd,
                        intern->attr->item_fg, (Cursor)None, EnterWindowMask | ExposureMask, 0);
    }
    if (entry->sub_entrys) {
        sub_win = make_pane(intern, entry->sub_title, (xpa_entry*)entry->sub_entrys, lev + 1);
    } else {
        sub_win = (Window)0;
    }
    item = new_item(item_win, entry->item_name, entry->key_char, lev, idx, width, exp_h, bl,
                    yspot + bl, key_off, sub_win, (xpa_pb*)0);
    if (entry->sub_entrys) {
        item->pb = make_pullbox(intern, item_win, item, width - XPA_HORPAD - XPA_PB_W,
                                exp_h - 2 /*XXXXX*/);
    }
 ...
 */
{
    ...
    for (idx = entrys; idx->item_name; idx++) {
        if (idx->item_type == XPA_ITEM_TYPE_ENTRY) {
            item_size(intern->attr->item_font, idx->item_name, (idx->sub_entrys != (xpa_entrys)0),
                      intern->attr->key_font, idx->key_char, &tw, &th, &bl, &off);
    ...
    /* Make window */
    result =
        make_window(intern->disp, DefaultRootWindow(intern->disp), 0, 0, w + key_off, h, XPA_BORDER,
                    intern->attr->border_bd, intern->attr->title_bg, intern->cursor,
                    LeaveWindowMask | ButtonPressMask | ButtonReleaseMask, SAVEUNDER | OVERRIDE);
 ...
 */
{
    ...
    if (mask & !passed)
        xpa_raise(XPA_ZAPP);
    rtn_app.title_font = def_font(disp, mask, XPA_T_FONT, PF(passed, title_font), &app_init,
                                  &(def_app.title_font), "*-bold-r-*-140-*-iso8859-*");
    rtn_app.item_font = def_font(disp, mask, XPA_I_FONT, PF(passed, item_font), &app_init,
                                 &(def_app.item_font), "*-medium-r-*-120-*-iso8859-*"
                                 /*		"-adobe-times-medium-r-*-*-16-*-*-*-p-*-*-*"*/);
    rtn_app.key_font = def_font(disp, mask, XPA_K_FONT, PF(passed, key_font), &app_init,
                                &(def_app.key_font), "*-medium-r-*-100-*-iso8859-*");

    rtn_app.title_fg = def_pixel(disp, mask, XPA_T_FG, PF(passed, title_fg), &app_init,
                                 &(def_app.title_fg), "white");
    rtn_app.title_bg = def_pixel(disp, mask, XPA_T_BG, PF(passed, title_bg), &app_init,
                                 &(def_app.title_bg), "black");
    rtn_app.item_fg = def_pixel(disp, mask, XPA_I_FG, PF(passed, item_fg), &app_init,
                                &(def_app.item_fg), "black");
    rtn_app.item_bg = def_pixel(disp, mask, XPA_I_BG, PF(passed, item_bg), &app_init,
                                &(def_app.item_bg), "white");
    rtn_app.border_bd = def_pixel(disp, mask, XPA_I_FG, PF(passed, border_bd), &app_init,
                                  &(def_app.border_bd), "orange"); /*XXX*/
    rtn_app.cur_fg =
        def_pixel(disp, mask, XPA_C_FG, PF(passed, cur_fg), &app_init, &(def_app.cur_fg), "black");
    return &rtn_app;
}
 ...
 */
{
    ...
    if (res_alloc <= intern->depth) {
        ...
        res_ary = (int*)realloc((char*)res_ary, (unsigned)(sizeof(int) * res_alloc));
    }
    res_len = 0;
    for (i = 0; i < intern->depth; i++) {
    ...
    if ((res_len == intern->depth) || ((res_len > 0) && (!intern->state[res_len].pane))) {
        *ary = res_ary;
    } else {
        *ary = (int*)0;
        res_len = 0;
 ...
 */
{
    ...
    if (!XFindContext(evt->xany.display, evt->xany.window, xpa_xcon, (caddr_t*)&cb)) {
        /* Found it */
        if (cb->callback) {
 ...
 */
{
    ...
    if (XGrabPointer(intern->disp, intern->top, True, ButtonPressMask | ButtonReleaseMask,
                     GrabModeAsync, GrabModeAsync, None, intern->cursor,
